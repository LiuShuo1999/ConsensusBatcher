...
static void dom2(bool ph,int cl,octet *DOM) {
    OCT_jstring(DOM,(char *)"SigBN158 no BN158 collisions");
    if (ph)
        OCT_jbyte(DOM,1,1);
    else
        OCT_jbyte(DOM,0,1);
...
{
#if CURVE_SECURITY_BN158 <= 128  // for ed25519?
    hash512 sh512;
    HASH512_init(&sh512);
...
static int H2(bool ph,octet *context,octet *R,octet *Q,octet *M,DBIG_160_28 dr)
{
    char h[128];
    int b=Q->len;
    char dom[64];
...
#endif
    reverse(2*b,h);
    BIG_160_28_dfromBytesLen(dr,h,2*b);
    return 2*b;
}

static void getR(bool ph,int b,char *digest,octet *context,octet *M,DBIG_160_28 dr) {
    char h[128];
    char dom[64];
    octet DOM={0,sizeof(dom),dom};
    int cl;
...
static int encode_int(BIG_160_28 x,char *ei) {
    int b,index=0;

    if (8*MODBYTES_160_28==MBITS_BN158) index=1; // extra byte needed for compression        
    b=MODBYTES_160_28+index;

    ei[0]=0;
    BIG_160_28_toBytes(&ei[index],x);
    reverse(b,ei);
    return b;
}

// encode point
static void encode(ECP_BN158 *P,octet *W) {
    BIG_160_28 x,y;
    int b,index=0;
    ...
    W->val[b-1]|=BIG_160_28_parity(x)<<7;
    W->len=b;
}

// decode integer (little endian)
static int decode_int(bool strip_sign,char *ei,BIG_160_28 x) {
    int b,index=0;
    int sign=0;
    char r[MODBYTES_160_28+1];
    ...
    if (r[0]==0) // remove leading zero
        BIG_160_28_fromBytesLen(x,&r[1],b-1);
    else
        BIG_160_28_fromBytesLen(x,&r[0],b);
    return sign;
}

// decode compressed point
static void decode(octet *W,ECP_BN158 *P) {
    BIG_160_28 X,Y;
    FP_BN158 x,d,t,one,hint;
    int sign=0;  // LSB of x

    sign=decode_int(true,W->val,Y);
    FP_BN158_nres(&x,Y); FP_BN158_sqr(&x,&x);
    FP_BN158_copy(&d,&x); FP_BN158_one(&one);
    FP_BN158_sub(&x,&x,&one);
    FP_BN158_norm(&x);
    FP_BN158_rcopy(&t, CURVE_B_BN158);
    FP_BN158_mul(&d,&d,&t);
#if CURVE_A_BN158 == 1
    FP_BN158_sub(&d,&d,&one);
#else
    FP_BN158_add(&d,&d,&one);
#endif    
    FP_BN158_norm(&d);
// inverse square root trick for sqrt(x/d)
    ...
    FP_BN158_mul(&x,&x,&t);
    FP_BN158_mul(&x,&x,&d);
    if (!FP_BN158_qr(&x,&hint))
    {
        ECP_BN158_inf(P);
        return;
    }
    FP_BN158_sqrt(&d,&x,&hint);
    FP_BN158_inv(&x,&x,&hint);
    FP_BN158_mul(&x,&x,&d);
    FP_BN158_mul(&x,&x,&t);

    FP_BN158_reduce(&x);
    FP_BN158_redc(X,&x);
    if (BIG_160_28_parity(X)!=sign)
        FP_BN158_neg(&x,&x);
    FP_BN158_norm(&x);
    FP_BN158_redc(X,&x);
    ECP_BN158_set(P,X,Y);
 ...
 */
int EDDSA_BN158_KEY_PAIR_GENERATE(csprng *RNG,octet* D,octet *Q)
{
    BIG_160_28 r, x, y, s;
    ECP_BN158 G;
    char digest[128];
...
// reverse bytes for little endian
    reverse(b,digest);
    BIG_160_28_fromBytes(s,&digest[index]);
    RFC7748(s);
    ECP_BN158_mul(&G, s);
...
int EDDSA_BN158_SIGNATURE(bool ph,octet *D, octet *context,octet *M,octet *SIG)
{
    DBIG_160_28 dr;
    BIG_160_28 s,sr,sd,q;
    ECP_BN158 R;
    char digest[128];
    char pq[MODBYTES_160_28+1];
    ...
    EDDSA_BN158_KEY_PAIR_GENERATE(NULL,D,&Q);

    BIG_160_28_rcopy(q, CURVE_Order_BN158);
    ECP_BN158_generator(&R);
...
// reverse bytes for little endian        
        ...
        BIG_160_28_dmod(sd,dr,q);
        BIG_160_28_modmul(s,s,sd,q);
        BIG_160_28_modadd(s,s,sr,q);
...
bool EDDSA_BN158_VERIFY(bool ph,octet *Q,octet *context,octet *M,octet *SIG) 
{
    DBIG_160_28 du;
    BIG_160_28 q,t,su;
    ECP_BN158 R,QD,G;
    ...
    decode(SIG,&R);
    if (ECP_BN158_isinf(&R)) return false;
    decode_int(false,&SIG->val[b],t);
    if (BIG_160_28_comp(t,q)>=0) return false;
    decode(Q,&QD);
    if (ECP_BN158_isinf(&QD)) return false;

    H2(ph,context,SIG,Q,M,du);
    BIG_160_28_dmod(su,du,q);
    ECP_BN158_generator(&G);
    for (int i=0;i<lg;i++)
    { // use cofactor 2^c
        ECP_BN158_dbl(&G); ECP_BN158_dbl(&QD); ECP_BN158_dbl(&R);
    }

    ECP_BN158_neg(&QD);
    ECP_BN158_mul2(&G,&QD,t,su);

    if (!ECP_BN158_equals(&G,&R)) return false;
