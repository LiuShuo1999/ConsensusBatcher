...
void VAL_CBC_SK_sign(u8 id){
	if(ACK_VAL_CBC_Number[id] != Block_Num){while(1);}
	// ID -> id
	//if(ACK_ABA_share[round][ID])return;
	//h ** sk
	char digest[50];for(int i=0;i<50;i++)digest[i] = NULL;
	char CBC_Str[1000];for(int i=0;i<1000;i++)CBC_Str[i] = NULL;
	u8 packet_idx = 0;
	...
	hash256 sh256;
	HASH256_init(&sh256);
	for (int i=0;i < Block_Num*CBC_block_Size;i++) HASH256_process(&sh256,CBC_Str[i]);
    HASH256_hash(&sh256,digest);
