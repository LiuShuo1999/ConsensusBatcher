...
#endif // CURVETYPE_BN254!=MONTGOMERY
...
static void ECP_BN254_select(ECP_BN254 *P, ECP_BN254 W[], sign32 b)
{
    ...
    ECP_BN254_cmove(P, &W[0], teq(babs, 0)); // conditional move
    ECP_BN254_cmove(P, &W[1], teq(babs, 1));
    ECP_BN254_cmove(P, &W[2], teq(babs, 2));
    ECP_BN254_cmove(P, &W[3], teq(babs, 3));
    ECP_BN254_cmove(P, &W[4], teq(babs, 4));
    ECP_BN254_cmove(P, &W[5], teq(babs, 5));
    ECP_BN254_cmove(P, &W[6], teq(babs, 6));
    ECP_BN254_cmove(P, &W[7], teq(babs, 7));
...
void ECP_BN254_rhs(FP_BN254 *v, FP_BN254 *x)
{
    ...
    FP_BN254_mul(&t, v, &t);
    FP_BN254_sub(&t, &t, &one);
    FP_BN254_norm(&t);
#if CURVE_A_BN254 == 1
        FP_BN254_sub(v, v, &one);
#endif

#if CURVE_A_BN254 == -1
        FP_BN254_add(v, v, &one);
        FP_BN254_norm(v);
        FP_BN254_neg(v, v);
#endif
    FP_BN254_norm(v);
    FP_BN254_inv(&t, &t, NULL);
    FP_BN254_mul(v, v, &t);
    FP_BN254_reduce(v);
#endif
    ...
    FP_BN254 x2, x3;
    FP_BN254_sqr(&x2, x);
    FP_BN254_mul(&x3, &x2, x);
    FP_BN254_copy(v, x);
    FP_BN254_imul(&x2, &x2, CURVE_A_BN254);
    FP_BN254_add(v, v, &x2);
    FP_BN254_add(v, v, &x3);
    FP_BN254_reduce(v);
#endif
...
int ECP_BN254_set(ECP_BN254 *P, BIG_256_28 x)
{
    //BIG_256_28 m, b;
    FP_BN254 rhs;
...
int ECP_BN254_get(BIG_256_28 x, ECP_BN254 *P)
{
    ECP_BN254 W;
    ECP_BN254_copy(&W, P);
    ECP_BN254_affine(&W);
...
/* SU=16 */
int ECP_BN254_get(BIG_256_28 x, BIG_256_28 y, ECP_BN254 *P)
{
    ECP_BN254 W;
    ECP_BN254_copy(&W, P);
    ECP_BN254_affine(&W);
    ...
    return FP_BN254_sign(&(W.x));
}

/* Set P=(x,{y}) */
/* SU=96 */
int ECP_BN254_set(ECP_BN254 *P, BIG_256_28 x, BIG_256_28 y)
{
    FP_BN254 rhs, y2;
...
/* SU=136 */
int ECP_BN254_setx(ECP_BN254 *P, BIG_256_28 x, int s)
{
    FP_BN254 rhs,hint;
    FP_BN254_nres(&rhs, x);
    ...
    FP_BN254_nres(&(P->x), x);
    FP_BN254_sqrt(&(P->y), &rhs,&hint);

    if (FP_BN254_sign(&(P->y))!=s)
        FP_BN254_neg(&(P->y), &(P->y));
    FP_BN254_reduce(&(P->y));
    FP_BN254_one(&(P->z));
    return 1;
...
{   /* multiply point by curves cofactor */
    ...
    BIG_256_28_rcopy(c, CURVE_Cof_BN254);
    ECP_BN254_mul(P, c);
    return;
}
...
/*
void ECP_BN254_hap2point(ECP_BN254 *P,BIG_256_28 h)
{
    BIG_256_28 x;
    BIG_256_28_copy(x,h);
...
void ECP_BN254_map2point(ECP_BN254 *P,FP_BN254 *h)
{
#if CURVETYPE_BN254==MONTGOMERY
// Elligator 2
    int qres;
    BIG_256_28 a;
    FP_BN254 X1,X2,w,t,one,A,N,D,hint;
    //BIG_256_28_zero(a); BIG_256_28_inc(a,CURVE_A_BN254); BIG_256_28_norm(a); FP_BN254_nres(&A,a);
    FP_BN254_from_int(&A,CURVE_A_BN254);
    FP_BN254_copy(&t,h);
    FP_BN254_sqr(&t,&t);   // t^2

    if (PM1D2_BN254 == 2)
        FP_BN254_add(&t,&t,&t);  // 2t^2
    if (PM1D2_BN254 == 1)
        FP_BN254_neg(&t,&t);      // -t^2
    if (PM1D2_BN254 > 2)
        FP_BN254_imul(&t,&t,QNRI_BN254); // precomputed QNR
    FP_BN254_norm(&t);  // z.t^2

    FP_BN254_one(&one);
    FP_BN254_add(&D,&t,&one); FP_BN254_norm(&D);  // Denominator D=1+z.t^2

    FP_BN254_copy(&X1,&A);
    FP_BN254_neg(&X1,&X1);  FP_BN254_norm(&X1);  // X1 = -A/D
    FP_BN254_copy(&X2,&X1);
    FP_BN254_mul(&X2,&X2,&t);             // X2 = -At/D

    FP_BN254_sqr(&w,&X1); FP_BN254_mul(&N,&w,&X1);  // w=X1^2, N=X1^3
    FP_BN254_mul(&w,&w,&A); FP_BN254_mul(&w,&w,&D); FP_BN254_add(&N,&N,&w);  // N = X1^3+ADX1^2
    FP_BN254_sqr(&t,&D);
    FP_BN254_mul(&t,&t,&X1);  
    FP_BN254_add(&N,&N,&t);  // N=X1^3+ADX1^2+D^2X1  // Numerator of gx =  N/D^3
    FP_BN254_norm(&N);

    FP_BN254_mul(&t,&N,&D);                   // N.D
    qres=FP_BN254_qr(&t,&hint);  // *** exp
    FP_BN254_inv(&w,&t,&hint);
    FP_BN254_mul(&D,&w,&N);     // 1/D
    FP_BN254_mul(&X1,&X1,&D);    // get X1
    FP_BN254_mul(&X2,&X2,&D);    // get X2
    FP_BN254_cmove(&X1,&X2,1-qres);
    FP_BN254_redc(a,&X1);
...
// Elligator 2 - map to Montgomery, place point, map back
    int qres,ne,rfc,qnr;
    BIG_256_28 x,y;
    FP_BN254 X1,X2,t,w,one,A,w1,w2,B,Y,K,D,hint;
    FP_BN254_one(&one);
...
// Figure out the Montgomery curve parameters
    ...
    FP_BN254_rcopy(&B,CURVE_B_BN254);
#if CURVE_A_BN254 ==  1
        FP_BN254_add(&A,&B,&one);  // A=B+1
        FP_BN254_sub(&B,&B,&one);   // B=B-1
#else
        FP_BN254_sub(&A,&B,&one);  // A=B-1
        FP_BN254_add(&B,&B,&one);  // B=B+1
#endif
    FP_BN254_norm(&A); FP_BN254_norm(&B);

    FP_BN254_div2(&A,&A);    // (A+B)/2
    FP_BN254_div2(&B,&B);    // (B-A)/2
    FP_BN254_div2(&B,&B);    // (B-A)/4

    FP_BN254_neg(&K,&B); FP_BN254_norm(&K);
    //FP_BN254_inv(&K,&K,NULL);    // K
    FP_BN254_invsqrt(&K,&w1,&K);

    rfc=RIADZ_BN254;
    if (rfc)
    { // RFC7748 method applies
        FP_BN254_mul(&A,&A,&K);   // = J
        FP_BN254_mul(&K,&K,&w1);
        //FP_BN254_sqrt(&K,&K,NULL);
    } else { // generic method
        FP_BN254_sqr(&B,&B);
    }
#else
    FP_BN254_from_int(&A,156326);
    rfc=1;
#endif
// Map to this Montgomery curve X^2=X^3+AX^2+BX
    ...
    FP_BN254_norm(&t);
    FP_BN254_add(&D,&t,&one); FP_BN254_norm(&D);  // Denominator=(1+z.u^2)

    FP_BN254_copy(&X1,&A);
    FP_BN254_neg(&X1,&X1);  FP_BN254_norm(&X1);    // X1=-(J/K).inv(1+z.u^2)
    FP_BN254_mul(&X2,&X1,&t);  // X2=X1*z.u^2

// Figure out RHS of Montgomery curve in rational form gx1/d^3

    FP_BN254_sqr(&w,&X1); FP_BN254_mul(&w1,&w,&X1);  // w=X1^2, w1=X1^3
    FP_BN254_mul(&w,&w,&A); FP_BN254_mul(&w,&w,&D); FP_BN254_add(&w1,&w1,&w);  // w1 = X1^3+ADX1^2
    FP_BN254_sqr(&w2,&D);
    if (!rfc)
    {
        FP_BN254_mul(&w,&X1,&B);
        FP_BN254_mul(&w2,&w2,&w); //
        FP_BN254_add(&w1,&w1,&w2);   // w1=X1^3+ADX1^2+BD^2X1
    } else {
        FP_BN254_mul(&w2,&w2,&X1);  
        FP_BN254_add(&w1,&w1,&w2);  // w1=X1^3+ADX1^2+D^2X1  // was &X1
    }
    FP_BN254_norm(&w1);

    FP_BN254_mul(&B,&w1,&D);     // gx1=num/den^3 - is_qr num*den (same as num/den, same as num/den^3)
    qres=FP_BN254_qr(&B,&hint);  // ***
    FP_BN254_inv(&w,&B,&hint);
    FP_BN254_mul(&D,&w,&w1);     // 1/D
    FP_BN254_mul(&X1,&X1,&D);    // get X1
    FP_BN254_mul(&X2,&X2,&D);    // get X2
    FP_BN254_sqr(&D,&D);

    FP_BN254_imul(&w1,&B,qnr);       // now for gx2 = Z.u^2.gx1
    FP_BN254_rcopy(&w,CURVE_HTPC_BN254);   // qnr^C3  
    FP_BN254_mul(&w,&w,&hint);    // modify hint for gx2
    FP_BN254_mul(&w2,&D,h);

    FP_BN254_cmove(&X1,&X2,1-qres);  // pick correct one
    FP_BN254_cmove(&B,&w1,1-qres);
    FP_BN254_cmove(&hint,&w,1-qres);
    FP_BN254_cmove(&D,&w2,1-qres);
     
    FP_BN254_sqrt(&Y,&B,&hint);   // sqrt(num*den)
    FP_BN254_mul(&Y,&Y,&D);       // sqrt(num/den^3)

/*
    FP_BN254_sqrt(&Y,&B,&hint);   // sqrt(num*den)
    FP_BN254_mul(&Y,&Y,&D);       // sqrt(num/den^3)

    FP_BN254_imul(&B,&B,qnr);     // now for gx2 = Z.u^2.gx1
    FP_BN254_rcopy(&w,CURVE_HTPC_BN254);   // qnr^C3  
    FP_BN254_mul(&hint,&hint,&w);    // modify hint for gx2

    FP_BN254_sqrt(&Y3,&B,&hint);  // second candidate
    FP_BN254_mul(&D,&D,h);
    FP_BN254_mul(&Y3,&Y3,&D);

    FP_BN254_cmove(&X1,&X2,1-qres);  // pick correct one
    FP_BN254_cmove(&Y,&Y3,1-qres);
*/
// correct sign of Y
    FP_BN254_neg(&w,&Y); FP_BN254_norm(&w);
    FP_BN254_cmove(&Y,&w,qres^FP_BN254_sign(&Y));

    if (!rfc)
    {
        FP_BN254_mul(&X1,&X1,&K);
        FP_BN254_mul(&Y,&Y,&K);
    }

#if MODTYPE_BN254 == GENERALISED_MERSENNE
// Ed448 isogeny
    FP_BN254_sqr(&t,&X1);  // t=u^2
    FP_BN254_add(&w,&t,&one); // w=u^2+1
    FP_BN254_norm(&w);
    FP_BN254_sub(&t,&t,&one); // t=u^2-1
    FP_BN254_norm(&t);
    FP_BN254_mul(&w1,&t,&Y);  // w1=v(u^2-1)
    FP_BN254_add(&w1,&w1,&w1);
    FP_BN254_add(&X2,&w1,&w1);
    FP_BN254_norm(&X2);       // w1=4v(u^2-1)
    FP_BN254_sqr(&t,&t);      // t=(u^2-1)^2
    FP_BN254_sqr(&Y,&Y);      // v^2
    FP_BN254_add(&Y,&Y,&Y);
    FP_BN254_add(&Y,&Y,&Y);
    FP_BN254_norm(&Y);        // 4v^2
    FP_BN254_add(&B,&t,&Y);  // w2=(u^2-1)^2+4v^2
    FP_BN254_norm(&B);                                   // X1=w1/w2 - X2=w1, B=w2

    FP_BN254_sub(&w2,&Y,&t);   // w2=4v^2-(u^2-1)^2
    FP_BN254_norm(&w2);
    FP_BN254_mul(&w2,&w2,&X1); // w2=u(4v^2-(u^2-1)^2)
    FP_BN254_mul(&t,&t,&X1);   // t=u(u^2-1)^2
    FP_BN254_div2(&Y,&Y);      // 2v^2
    FP_BN254_mul(&w1,&Y,&w);  // w1=2v^2(u^2+1)
    FP_BN254_sub(&w1,&t,&w1);  // w1=u(u^2-1)^2 - 2v^2(u^2+1)
    FP_BN254_norm(&w1);                                   // Y=w2/w1

    FP_BN254_mul(&t,&X2,&w1);    // output in projective to avoid inversion
    FP_BN254_copy(&(P->x),&t);
    FP_BN254_mul(&t,&w2,&B);
    FP_BN254_copy(&(P->y),&t);
    FP_BN254_mul(&t,&w1,&B);
...
#else
    FP_BN254_add(&w1,&X1,&one); FP_BN254_norm(&w1); // (s+1)
    FP_BN254_sub(&w2,&X1,&one); FP_BN254_norm(&w2); // (s-1)
    FP_BN254_mul(&t,&w1,&Y);
    FP_BN254_mul(&X1,&X1,&w1);

    if (rfc)
        FP_BN254_mul(&X1,&X1,&K);

    FP_BN254_mul(&Y,&Y,&w2);      // output in projective to avoid inversion
    FP_BN254_copy(&(P->x),&X1);
    FP_BN254_copy(&(P->y),&Y);
    FP_BN254_copy(&(P->z),&t);
    return;
#endif
    ...
    int sgn,ne;
    BIG_256_28 a,x,y;
    FP_BN254 X1,X2,X3,t,w,one,A,B,Y,D;
    FP_BN254 D2,hint,GX1;

#if HTC_ISO_BN254 != 0
// Map to point on isogenous curve
    int i,k,isox,isoy,iso=HTC_ISO_BN254;
    FP_BN254 xnum,xden,ynum,yden;
    BIG_256_28 z;
    FP_BN254_rcopy(&A,CURVE_Ad_BN254);
    FP_BN254_rcopy(&B,CURVE_Bd_BN254);
#else
    FP_BN254_from_int(&A,CURVE_A_BN254);
    FP_BN254_rcopy(&B,CURVE_B_BN254);
#endif
...
#if CURVE_A_BN254 != 0 || HTC_ISO_BN254 != 0

        FP_BN254_sqr(&t,&t);
        FP_BN254_imul(&t,&t,RIADZ_BN254);  // Z from hash-to-point draft standard
        FP_BN254_add(&w,&t,&one);     // w=Zt^2+1
        FP_BN254_norm(&w);

        FP_BN254_mul(&w,&w,&t);      // w=Z^2*t^4+Zt^2
        FP_BN254_mul(&D,&A,&w);      // A=Aw
                               
        FP_BN254_add(&w,&w,&one); FP_BN254_norm(&w);
        FP_BN254_mul(&w,&w,&B);
        FP_BN254_neg(&w,&w);          // -B(w+1)
        FP_BN254_norm(&w);

        FP_BN254_copy(&X2,&w);        // Numerators
        FP_BN254_mul(&X3,&t,&X2);

// x^3+Ad^2x+Bd^3
        FP_BN254_sqr(&GX1,&X2);
        FP_BN254_sqr(&D2,&D); FP_BN254_mul(&w,&A,&D2); FP_BN254_add(&GX1,&GX1,&w); FP_BN254_norm(&GX1); FP_BN254_mul(&GX1,&GX1,&X2); FP_BN254_mul(&D2,&D2,&D); FP_BN254_mul(&w,&B,&D2); FP_BN254_add(&GX1,&GX1,&w); FP_BN254_norm(&GX1);

        FP_BN254_mul(&w,&GX1,&D);
        int qr=FP_BN254_qr(&w,&hint);   // qr(ad) - only exp happens here
        FP_BN254_inv(&D,&w,&hint);     // d=1/(ad)
        FP_BN254_mul(&D,&D,&GX1);     // 1/d
        FP_BN254_mul(&X2,&X2,&D);     // X2/=D
        FP_BN254_mul(&X3,&X3,&D);     // X3/=D
        FP_BN254_mul(&t,&t,h);        // t=Z.u^3
        FP_BN254_sqr(&D2,&D);

        FP_BN254_mul(&D,&D2,&t);
        FP_BN254_imul(&t,&w,RIADZ_BN254);
        FP_BN254_rcopy(&X1,CURVE_HTPC_BN254);     
        FP_BN254_mul(&X1,&X1,&hint); // modify hint

        FP_BN254_cmove(&X2,&X3,1-qr); 
        FP_BN254_cmove(&D2,&D,1-qr);
        FP_BN254_cmove(&w,&t,1-qr);
        FP_BN254_cmove(&hint,&X1,1-qr);

        FP_BN254_sqrt(&Y,&w,&hint);  // first candidate if X2 is correct
        FP_BN254_mul(&Y,&Y,&D2);
...
/*
        ...
        FP_BN254_mul(&D2,&D2,&t);     // second candidate if X3 is correct
        FP_BN254_imul(&w,&w,RIADZ_BN254); 

        FP_BN254_rcopy(&X1,CURVE_HTPC_BN254);     
        FP_BN254_mul(&hint,&hint,&X1); // modify hint

        FP_BN254_sqrt(&Y3,&w,&hint);
        FP_BN254_mul(&Y3,&Y3,&D2);

        FP_BN254_cmove(&X2,&X3,!qr); 
        FP_BN254_cmove(&Y,&Y3,!qr); 
*/
        ne=FP_BN254_sign(&Y)^sgn;
        FP_BN254_neg(&w,&Y); FP_BN254_norm(&w);
        FP_BN254_cmove(&Y,&w,ne);
...
// xnum
        FP_BN254_rcopy(&xnum,PC_BN254[k++]);
        for (i=0;i<isox;i++)
        {
            FP_BN254_mul(&xnum,&xnum,&X2); 
            FP_BN254_rcopy(&w,PC_BN254[k++]);
            FP_BN254_add(&xnum,&xnum,&w); FP_BN254_norm(&xnum);
        }

// xden
        FP_BN254_copy(&xden,&X2);
        FP_BN254_rcopy(&w,PC_BN254[k++]);
        FP_BN254_add(&xden,&xden,&w); FP_BN254_norm(&xden);
 
        for (i=0;i<isox-2;i++)
        {
            FP_BN254_mul(&xden,&xden,&X2);
            FP_BN254_rcopy(&w,PC_BN254[k++]);
            FP_BN254_add(&xden,&xden,&w); FP_BN254_norm(&xden);
        }

// ynum
        FP_BN254_rcopy(&ynum,PC_BN254[k++]);
        for (i=0;i<isoy;i++)
        {
            FP_BN254_mul(&ynum,&ynum,&X2); 
            FP_BN254_rcopy(&w,PC_BN254[k++]);
            FP_BN254_add(&ynum,&ynum,&w); FP_BN254_norm(&ynum);
        }

// yden 
        FP_BN254_copy(&yden,&X2);
        FP_BN254_rcopy(&w,PC_BN254[k++]);
        FP_BN254_add(&yden,&yden,&w); FP_BN254_norm(&yden);
      
        for (i=0;i<isoy-1;i++)
        {
            FP_BN254_mul(&yden,&yden,&X2); 
            FP_BN254_rcopy(&w,PC_BN254[k++]);
            FP_BN254_add(&yden,&yden,&w); FP_BN254_norm(&yden);
        }

        FP_BN254_mul(&ynum,&ynum,&Y);

// return in projective coordinates
        FP_BN254_mul(&t,&xnum,&yden);
        FP_BN254_copy(&(P->x),&t);

        FP_BN254_mul(&t,&ynum,&xden);
        FP_BN254_copy(&(P->y),&t);

        FP_BN254_mul(&t,&xden,&yden);
        FP_BN254_copy(&(P->z),&t);
        return;
#else
...
// SVDW - Shallue and van de Woestijne
        FP_BN254_from_int(&Y,RIADZ_BN254);
        ECP_BN254_rhs(&A,&Y);  // A=g(Z)
        FP_BN254_rcopy(&B,SQRTm3_BN254);
        FP_BN254_imul(&B,&B,RIADZ_BN254); // Z*sqrt(-3)

        FP_BN254_sqr(&t,&t);
        FP_BN254_mul(&Y,&A,&t);   // tv1=u^2*g(Z)
        FP_BN254_add(&t,&one,&Y); FP_BN254_norm(&t); // tv2=1+tv1
        FP_BN254_sub(&Y,&one,&Y); FP_BN254_norm(&Y); // tv1=1-tv1
        FP_BN254_mul(&D,&t,&Y);
        FP_BN254_mul(&D,&D,&B);

        FP_BN254_copy(&w,&A);
        FP_BN254_tpo(&D,&w);   // tv3=inv0(tv1*tv2*z*sqrt(-3)) and sqrt(g(Z))   // ***

        FP_BN254_mul(&w,&w,&B);  // tv4=Z.sqrt(-3).sqrt(g(Z))
        if (FP_BN254_sign(&w)==1)
        { // depends only on sign of constant RIADZ
            FP_BN254_neg(&w,&w);
            FP_BN254_norm(&w);
        }
        FP_BN254_mul(&w,&w,&B);  // Z.sqrt(-3)
        FP_BN254_mul(&w,&w,h);    // u
        FP_BN254_mul(&w,&w,&Y);   // tv1
        FP_BN254_mul(&w,&w,&D);  // tv3   // tv5=u*tv1*tv3*tv4*Z*sqrt(-3)

        FP_BN254_from_int(&X1,RIADZ_BN254);
        FP_BN254_copy(&X3,&X1);
        FP_BN254_neg(&X1,&X1); FP_BN254_norm(&X1); FP_BN254_div2(&X1,&X1); // -Z/2
        FP_BN254_copy(&X2,&X1);
        FP_BN254_sub(&X1,&X1,&w); FP_BN254_norm(&X1);
        FP_BN254_add(&X2,&X2,&w); FP_BN254_norm(&X2);
        FP_BN254_add(&A,&A,&A);
        FP_BN254_add(&A,&A,&A);
        FP_BN254_norm(&A);      // 4*g(Z)
        FP_BN254_sqr(&t,&t);
        FP_BN254_mul(&t,&t,&D);
        FP_BN254_sqr(&t,&t);    // (tv2^2*tv3)^2
        FP_BN254_mul(&A,&A,&t); // 4*g(Z)*(tv2^2*tv3)^2

        FP_BN254_add(&X3,&X3,&A); FP_BN254_norm(&X3);

        ECP_BN254_rhs(&w,&X2);
        FP_BN254_cmove(&X3,&X2,FP_BN254_qr(&w,NULL));                           // ***
        ECP_BN254_rhs(&w,&X1);
        FP_BN254_cmove(&X3,&X1,FP_BN254_qr(&w,NULL));                           // ***
        ECP_BN254_rhs(&w,&X3);
        FP_BN254_sqrt(&Y,&w,NULL);                                        // ***
        FP_BN254_redc(x,&X3);
...
void ECP_BN254_affine(ECP_BN254 *P)
{
    ...
    FP_BN254_inv(&iz, &(P->z),NULL);
    FP_BN254_mul(&(P->x), &(P->x), &iz);

#if CURVETYPE_BN254==EDWARDS || CURVETYPE_BN254==WEIERSTRASS
...
void ECP_BN254_toOctet(octet *W, ECP_BN254 *P, bool compress)
{
    ...
    if (alt)
    {
        ...
        if (compress)
        {
            ...
            W->val[0]|=0x80;
            if (FP_BN254_islarger(&(P->y))==1) W->val[0]|=0x20;
        } else {
        ...
        {
            W->val[0] = 0x02;
            if (FP_BN254_sign(&(P->y)) == 1) W->val[0] = 0x03;
            W->len = MODBYTES_256_28 + 1;
        } else {
            W->val[0] = 0x04;
...
int ECP_BN254_fromOctet(ECP_BN254 *P, octet *W)
{
    ...
    if (alt)
    {
        ...
        if ((typ&0x80)==0)
        {
            ...
            if (!ECP_BN254_setx(P,x,0)) return 0;
            sgn=(typ&0x20)>>5;
            cmp=FP_BN254_islarger(&(P->y));
            if ((sgn==1 && cmp!=1) || (sgn==0 && cmp==1)) ECP_BN254_neg(P);
            return 1;
        }
...
void ECP_BN254_dbl(ECP_BN254 *P)
{
    ...
    FP_BN254_sqr(&C, &(P->x));                        //C.sqr();

    FP_BN254_mul(&(P->x), &(P->x), &(P->y));      //x.mul(y);
    FP_BN254_add(&(P->x), &(P->x), &(P->x));      //x.add(x);
    FP_BN254_norm(&(P->x));                       //x.norm();

    FP_BN254_sqr(&D, &(P->y));                        //D.sqr();

#if CURVE_A_BN254 == -1
        FP_BN254_neg(&C, &C);             //  C.neg();
#endif
    FP_BN254_add(&(P->y), &C, &D);    //y.add(D);
    FP_BN254_norm(&(P->y));               //y.norm();
    FP_BN254_sqr(&H, &(P->z));            //H.sqr();
    FP_BN254_add(&H, &H, &H);             //H.add(H);

    FP_BN254_sub(&J, &(P->y), &H);        //J.sub(H);
    FP_BN254_norm(&J);                    //J.norm();

    FP_BN254_mul(&(P->x), &(P->x), &J);   //x.mul(J);
    FP_BN254_sub(&C, &C, &D);             //C.sub(D);
    FP_BN254_norm(&C);                    //C.norm();
    FP_BN254_mul(&(P->z), &(P->y), &J);   //z.mul(J);
    FP_BN254_mul(&(P->y), &(P->y), &C);   //y.mul(C);


#endif

#if CURVETYPE_BN254==MONTGOMERY
    FP_BN254 A, B, AA, BB, C;

    FP_BN254_add(&A, &(P->x), &(P->z));       //A.add(z);
    FP_BN254_norm(&A);                    //A.norm();
    FP_BN254_sqr(&AA, &A);            //AA.sqr();
    FP_BN254_sub(&B, &(P->x), &(P->z));       //B.sub(z);
    FP_BN254_norm(&B);                    //B.norm();

    FP_BN254_sqr(&BB, &B);            //BB.sqr();
    FP_BN254_sub(&C, &AA, &BB);           //C.sub(BB);
    FP_BN254_norm(&C);                    //C.norm();
    FP_BN254_mul(&(P->x), &AA, &BB);  //x.mul(BB);
    FP_BN254_imul(&A, &C, (CURVE_A_BN254 + 2) / 4); //A.imul((ROM.CURVE_A+2)/4);

    FP_BN254_add(&BB, &BB, &A);           //BB.add(A);
    FP_BN254_norm(&BB);                   //BB.norm();
    FP_BN254_mul(&(P->z), &BB, &C);   //z.mul(C);
...
void ECP_BN254_add(ECP_BN254 *P, ECP_BN254 *Q, ECP_BN254 *W)
{
    FP_BN254 A, B, C, D, DA, CB;

    FP_BN254_add(&A, &(P->x), &(P->z)); //A.add(z);
    FP_BN254_sub(&B, &(P->x), &(P->z)); //B.sub(z);

    FP_BN254_add(&C, &(Q->x), &(Q->z)); //C.add(Q.z);

    FP_BN254_sub(&D, &(Q->x), &(Q->z)); //D.sub(Q.z);
    FP_BN254_norm(&A);            //A.norm();

    FP_BN254_norm(&D);            //D.norm();
    FP_BN254_mul(&DA, &D, &A);        //DA.mul(A);

    FP_BN254_norm(&C);            //C.norm();
    FP_BN254_norm(&B);            //B.norm();
    FP_BN254_mul(&CB, &C, &B);    //CB.mul(B);

    FP_BN254_add(&A, &DA, &CB);   //A.add(CB);
    FP_BN254_norm(&A);            //A.norm();
    FP_BN254_sqr(&(P->x), &A);        //A.sqr();
    FP_BN254_sub(&B, &DA, &CB);   //B.sub(CB);
    FP_BN254_norm(&B);            //B.norm();
    FP_BN254_sqr(&B, &B);         //B.sqr();

    FP_BN254_mul(&(P->z), &(W->x), &B); //z.mul(B);
...
#else
    FP_BN254 A, B, C, D, E, F, G, b;

    FP_BN254_mul(&A, &(P->z), &(Q->z));   //A.mul(Q.z);
    FP_BN254_sqr(&B, &A);                 //B.sqr();
    FP_BN254_mul(&C, &(P->x), &(Q->x));   //C.mul(Q.x);
    FP_BN254_mul(&D, &(P->y), &(Q->y));   //D.mul(Q.y);
    FP_BN254_mul(&E, &C, &D);             //E.mul(D);

    if (CURVE_B_I_BN254 == 0)         //if (ROM.CURVE_B_I==0)
    {
        FP_BN254_rcopy(&b, CURVE_B_BN254);  //FP b=new FP(new BIG(ROM.CURVE_B));
        FP_BN254_mul(&E, &E, &b);         //E.mul(b);
    }
    else
        FP_BN254_imul(&E, &E, CURVE_B_I_BN254); //E.imul(ROM.CURVE_B_I);

    FP_BN254_sub(&F, &B, &E);         //F.sub(E);
    FP_BN254_add(&G, &B, &E);         //G.add(E);

#if CURVE_A_BN254 == 1
        FP_BN254_sub(&E, &D, &C);     //E.sub(C);
#endif
    FP_BN254_add(&C, &C, &D);         //C.add(D);
    FP_BN254_add(&B, &(P->x), &(P->y));   //B.add(y);

    FP_BN254_add(&D, &(Q->x), &(Q->y));   //D.add(Q.y);
    FP_BN254_norm(&B);                //B.norm();
    FP_BN254_norm(&D);                //D.norm();
    FP_BN254_mul(&B, &B, &D);         //B.mul(D);
    FP_BN254_sub(&B, &B, &C);         //B.sub(C);
    FP_BN254_norm(&B);                //B.norm();
    FP_BN254_norm(&F);                //F.norm();
    FP_BN254_mul(&B, &B, &F);         //B.mul(F);
    FP_BN254_mul(&(P->x), &A, &B); //x.mul(B);
    FP_BN254_norm(&G);                //G.norm();

#if CURVE_A_BN254 == 1
        FP_BN254_norm(&E);            //E.norm();
        FP_BN254_mul(&C, &E, &G);     //C.mul(G);
#endif
#if CURVE_A_BN254 == -1
        FP_BN254_norm(&C);            //C.norm();
        FP_BN254_mul(&C, &C, &G);     //C.mul(G);
#endif
    FP_BN254_mul(&(P->y), &A, &C); //y.mul(C);
    FP_BN254_mul(&(P->z), &F, &G); //z.mul(G);
...
void ECP_BN254_mul(ECP_BN254 *P,BIG_256_28 e)
{
    ECP_BN254_clmul(P,e,e);
}

// .. but this one does not (typically set maxe=r)
// Set P=e*P 
void ECP_BN254_clmul(ECP_BN254 *P, BIG_256_28 e, BIG_256_28 maxe)
{
    int max;
    BIG_256_28 cm;
#if CURVETYPE_BN254==MONTGOMERY
...
void ECP_BN254_muln(ECP_BN254 *P,int n,ECP_BN254 X[],BIG_256_28 e[])
{
    int i,j,k,nb;
    BIG_256_28 t,mt;
    ECP_BN254 S,R,B[16];
    ...
    { // Pippenger's algorithm
        ...
        for (j=0;j<n;j++)
        {
            ...
            ECP_BN254_add(&B[k],&X[j]);
        }
        ECP_BN254_inf(&R); ECP_BN254_inf(&S);
        for (j=15;j>=1;j--)
        {
            ECP_BN254_add(&R,&B[j]);
            ECP_BN254_add(&S,&R);
...
void ECP_BN254_mul2(ECP_BN254 *P, ECP_BN254 *Q, BIG_256_28 e, BIG_256_28 f)
{
    BIG_256_28 te, tf, mt;
    ECP_BN254 S, T, W[8], C;
...
int ECP_BN254_generator(ECP_BN254 *G)
{
    BIG_256_28 x, y;
    BIG_256_28_rcopy(x, CURVE_Gx_BN254);
#if CURVETYPE_BN254!=MONTGOMERY
    BIG_256_28_rcopy(y, CURVE_Gy_BN254);
    return ECP_BN254_set(G, x, y);
#else
    return ECP_BN254_set(G, x);
#endif
}
