...
//				sprintf((char*)Send_Data_buff,"STATE: Get EVENT 1.");LoRa_SendData(Send_Data_buff);
			...
			{
				u8 er_flag = 1;
				if(RBC_done_Number < parallel_D || need_EVENT_RBC_ER || RBC_init_Number < parallel_D){
					need_EVENT_RBC_ER = 0;
					RBC_ER_handler();
					er_flag = 0;
				}
				...
				if(PRBC_done_Number < Nodes_N && RBC_done_Number >= parallel_D && RBC_init_Number == parallel_D ){
...
void RBC_init(){
	...
	RBC_done_Number = 0;
	RBC_init_Number = 0;
	if(ID <= parallel_D)RBC_init_Number++;
	///PRBC
	for(int i=0;i<MAX_Nodes;i++){
...
void PRBC_SIG_handler(){
	...
	if(Nodes_N <= 8){
		...
		u8 padding = 0;
		u8 sig_num = 0;for(int k=1;k<=parallel_D;k++)if(ACK_PRBC_share[ID][k] == 1)sig_num++;
		if(sig_num <= 2) padding = 80;
		else padding = 20;
...
//	if(ReceBuff[0] == 0x00||ReceBuff[0] > Nodes_N || ReceBuff[1] > 20 || ReceBuff[2] > Nodes_N){
	...
	if(packet_type == TypeRBC_INIT && verify_flag){
		...
		//VAL
		for(int k=1;k<=Nodes_N;k++){
			if(!check_list[k] && ID > parallel_D)continue;
...
//						ACK_PRBC_share[k][ID] = 1;
						...
						if(ACK_PRBC_share_Number[k] == 2 * Nodes_f + 1){
							...
							PRBC_done_Number++;
							if(ID == 1 && PRBC_done_Number == parallel_D && MY_Consensus_TIME==0){
								LoRa_CFG.chn = 55;
								LoRa_Set();	
								MY_Consensus_TIME = TIM2->CNT + 9000 * TIM2_Exceed_Times;
								sprintf((char*)Send_Data_buff,"PRBC-parallel-%d-%d",Block_Num,MY_Consensus_TIME);
								LoRa_SendData(Send_Data_buff);
								while(1);
							}
						}
	...
	else if(packet_type == TypePRBC_SIG && verify_flag && RBC_done_Number >= parallel_D){
		//DMA send STATE packet
		//只实现了16节点以下的情况。分情况讨论的
		u8 nack_flag = 0;
		u8 padding = buff[2];
			...
			if((sig_ack & (1<<(k-1))) != 0 && ACK_PRBC_share[k][s_id] == 0){// && ACK_PRBC_share[k][ID] == 1){
				...
				if(ACK_PRBC_share_Number[k] == 2 * Nodes_f + 1){
					...
					if(ID == 1 && PRBC_done_Number == parallel_D && MY_Consensus_TIME==0){
						...
						LoRa_Set();	
						MY_Consensus_TIME = TIM2->CNT + 9000 * TIM2_Exceed_Times;
						sprintf((char*)Send_Data_buff,"PRBC-parallel-%d-%d",parallel_D,MY_Consensus_TIME);
						LoRa_SendData(Send_Data_buff);
						while(1);
					}
