...
void CBC_STATE_handler(){
	...
	if(Nodes_N <= 8){
		...
		for(int k=1 ; k<=Nodes_N ; k++){
			if(k == ID)continue;
			if(ACK_CBC_Echo[k] && ID <= parallel_D){
				Send_Data_buff[packet_idx] |= (1<<(k-1));
			}	
		}
		if(ID > parallel_D) Send_Data_buff[packet_idx] = 0xFF;
		...
		//Finish nack
		for(int k=1 ; k<=Nodes_N ; k++){
			if(ACK_CBC_Finish[k] || k > parallel_D){
				Send_Data_buff[packet_idx] |= (1<<(k-1));
			}	
		}
...
//		if(My_CBC_Echo_Number >= 2 * Nodes_f + 1){
		if(My_CBC_Echo_Number >= 2 * Nodes_f + 1 && ID <= parallel_D){	
...
//	if(ReceBuff[0] == 0x00||ReceBuff[0] > Nodes_N || ReceBuff[1] > 20 || ReceBuff[2] > Nodes_N){
	...
	if(packet_type == TypeCBC_STATE && verify_flag){
		u8 nack_flag = 0;
		u8 padding = buff[2];
		u8 packet_idx = 3+padding;
		...
		//v
		if(CBC_Echo[s_id] == 0 && s_id <= parallel_D){
		...
		packet_idx += CBC_v_SIZE;
		for(int k=1;k<=Nodes_N;k++){
			if((echo_ack & (1<<(k-1))) != 0 && k == ID && ACK_CBC_Echo[s_id] == 0 && ID <= parallel_D){
				for(int i=0;i<Share_Size;i++){
				...
				if(My_CBC_Echo_Number >= 2 * Nodes_f + 1 && ID <= parallel_D){	
					if( ACK_CBC_Finish[ID] == 0){
						...
						ACK_CBC_Finish[ID] = 1;
						CBC_done_Number++;
						if(ID == 1 && CBC_done_Number == parallel_D && MY_Consensus_TIME==0){
							LoRa_CFG.chn = 77;
							LoRa_Set();	
							
							MY_Consensus_TIME = TIM2->CNT + 9000 * TIM2_Exceed_Times;
							sprintf((char*)Send_Data_buff,"CBC-small-%d-%d",parallel_D,MY_Consensus_TIME);
							LoRa_SendData(Send_Data_buff);
							while(1);
						}
					}
		...
		if((finish_nack & (1<<(s_id-1))) != 0 && ACK_CBC_Finish[s_id] == 0  && s_id <= parallel_D){
			for(int i=0;i<Share_Size;i++){
