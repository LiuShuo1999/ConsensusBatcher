...
int FP4_BLS12381_islarger(FP4_BLS12381 *x)
{
    int cmp;
    if (FP4_BLS12381_iszilch(x)) return 0;
    cmp=FP2_BLS12381_islarger(&(x->b));
    if (cmp!=0) return cmp;
    return FP2_BLS12381_islarger(&(x->a));
}

void FP4_BLS12381_toBytes(char *b,FP4_BLS12381 *x)
{
    FP2_BLS12381_toBytes(b,&(x->b));
    FP2_BLS12381_toBytes(&b[2*MODBYTES_384_29],&(x->a));
}

void FP4_BLS12381_fromBytes(FP4_BLS12381 *x,char *b)
{
    FP2_BLS12381_fromBytes(&(x->b),b);
    FP2_BLS12381_fromBytes(&(x->a),&b[2*MODBYTES_384_29]);
}
...
/* Set w=1/x */
/* SU= 160 */
void FP4_BLS12381_inv(FP4_BLS12381 *w, FP4_BLS12381 *x, FP_BLS12381 *h)
{
    FP2_BLS12381 t1, t2;
...
/* SU= 240 */
/*
void FP4_BLS12381_pow(FP4_BLS12381 *r, FP4_BLS12381* a, BIG_384_29 b)
{
    FP4_BLS12381 w;
    BIG_384_29 z, zilch;
    int bt;
...
void FP4_BLS12381_xtr_pow(FP4_BLS12381 *r, FP4_BLS12381 *x, BIG_384_29 n)
{
    int i, par, nb;
    BIG_384_29 v;
    FP2_BLS12381 w;
...
void FP4_BLS12381_xtr_pow2(FP4_BLS12381 *r, FP4_BLS12381 *ck, FP4_BLS12381 *cl, FP4_BLS12381 *ckml, FP4_BLS12381 *ckm2l, BIG_384_29 a, BIG_384_29 b)
{
    int i, f2;
    BIG_384_29 d, e, w;
...
/* test for x a QR */
int FP4_BLS12381_qr(FP4_BLS12381 *x, FP_BLS12381 *h)
{ /* test x^(p^4-1)/2 = 1 */
    FP4_BLS12381 c;
    FP4_BLS12381_conj(&c,x);
    FP4_BLS12381_mul(&c,&c,x);

    return FP2_BLS12381_qr(&(c.a),h);
}

/* sqrt(a+xb) = sqrt((a+sqrt(a*a-n*b*b))/2)+x.b/(2*sqrt((a+sqrt(a*a-n*b*b))/2)) */

void FP4_BLS12381_sqrt(FP4_BLS12381 *r, FP4_BLS12381* x, FP_BLS12381 *h)
{
    FP2_BLS12381 a, b, s, t;
    FP4_BLS12381 nr;
    FP_BLS12381 hint,twk;
    int sgn,qr;
    ...
    FP2_BLS12381_div2(&b,&(r->b));                   // w1=b/2
    qr=FP2_BLS12381_qr(&a,&hint);                    // only exp! Cost=+1

// tweak hint - multiply old hint by Norm(1/Beta)^e where Beta is irreducible polynomial
    FP2_BLS12381_copy(&s,&a);
    FP_BLS12381_rcopy(&twk,TWK_BLS12381);
    FP_BLS12381_mul(&twk,&twk,&hint);
    FP2_BLS12381_div_ip(&s); FP2_BLS12381_norm(&s); // switch to other candidate

    FP2_BLS12381_cmove(&a,&s,1-qr);
    FP_BLS12381_cmove(&hint,&twk,1-qr);

    FP2_BLS12381_sqrt(&(r->a),&a,&hint);             // a=sqrt(w2)  Cost=+1
    FP2_BLS12381_inv(&s,&a,&hint);                  // w3=1/w2
    FP2_BLS12381_mul(&s,&s,&(r->a));                // w3=1/sqrt(w2)
    FP2_BLS12381_mul(&(r->b),&s,&b);                // b=(b/2)*1/sqrt(w2)
    FP2_BLS12381_copy(&t,&(r->a));

    FP2_BLS12381_cmove(&(r->a),&(r->b),1-qr);
    FP2_BLS12381_cmove(&(r->b),&t,1-qr);
