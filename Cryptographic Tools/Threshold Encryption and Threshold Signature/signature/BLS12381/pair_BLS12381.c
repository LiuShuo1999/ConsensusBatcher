...
int PAIR_BLS12381_nbits(BIG_384_29 n3, BIG_384_29 n)
{
    BIG_384_29 x;
...
static void glv(BIG_384_29 u[2], BIG_384_29 ee)
{
    ...
    for (i = 0; i < 2; i++)
    {
        BIG_384_29_rcopy(t, CURVE_W_BLS12381[i]);
    ...
    for (i = 0; i < 2; i++)
        for (j = 0; j < 2; j++)
        {
            BIG_384_29_rcopy(t, CURVE_SB_BLS12381[j][i]);
...
static void gs(BIG_384_29 u[4], BIG_384_29 ee)
{
    ...
    for (i = 0; i < 4; i++)
    {
        BIG_384_29_rcopy(t, CURVE_WB_BLS12381[i]);
        BIG_384_29_mul(d, t, ee);
        BIG_384_29_ctddiv(v[i],d,q,BIG_384_29_nbits(t));
        BIG_384_29_zero(u[i]);
    }
    ...
    for (i = 0; i < 4; i++)
        for (j = 0; j < 4; j++)
        {
            BIG_384_29_rcopy(t, CURVE_BB_BLS12381[j][i]);
            BIG_384_29_modmul(t, v[j], t, q);
            BIG_384_29_add(u[i], u[i], q);
            BIG_384_29_sub(u[i], u[i], t);
            BIG_384_29_ctmod(u[i],q,1);
...
void PAIR_BLS12381_G1mul(ECP_BLS12381 *P, BIG_384_29 e)
{
    BIG_384_29 ee,q;
    ...
    BIG_384_29_rcopy(q, CURVE_Order_BLS12381);
    BIG_384_29_mod(ee,q);
    ECP_BLS12381_clmul(P, ee, q);
#endif
}

/* Multiply P by e in group G2 */
void PAIR_BLS12381_G2mul(ECP2_BLS12381 *P, BIG_384_29 e)
{
    BIG_384_29 ee,q;
    ...
    BIG_384_29_rcopy(q, CURVE_Order_BLS12381);
    BIG_384_29_mod(ee,q);
    ECP2_BLS12381_mul(P, ee);
#endif
}

/* f=f^e */
void PAIR_BLS12381_GTpow(FP12_BLS12381 *f, BIG_384_29 e)
{
    BIG_384_29 ee,q;
...
int PAIR_BLS12381_G1member(ECP_BLS12381 *P)
{
    ...
    ECP_BLS12381_mul(&T,x);
    if (ECP_BLS12381_equals(P,&T)) return 0;    // P is of low order   
    ECP_BLS12381_mul(&T,x);
    ECP_BLS12381_neg(&T);

    FP_BLS12381_rcopy(&cru, CRu_BLS12381);
    FP_BLS12381_mul(&(W.x), &(W.x), &cru);
    if (!ECP_BLS12381_equals(&W,&T)) return 0;  // check that Endomorphism works

// Not needed
//    ECP_BLS12381_add(&W,P);
//    FP_BLS12381_mul(&(T.x), &(T.x), &cru);
//    ECP_BLS12381_add(&W,&T);
//    if (!ECP_BLS12381_isinf(&W)) return 0;      // use it to check order
...
//https://eprint.iacr.org/2022/348.pdf
    ECP2_BLS12381_copy(&W,&T);
    ECP2_BLS12381_frob(&W,&X); // W=\psi(xP)
    ECP2_BLS12381_add(&T,P); // T=xP+P
    ECP2_BLS12381_add(&T,&W); // T=xP+P+\psi(xP)
    ECP2_BLS12381_frob(&W,&X); // W=\psi^2(xP)
    ECP2_BLS12381_add(&T,&W); // T=xp+P+\psi(xP)+\psi^2(xP)
    ECP2_BLS12381_frob(&W,&X); // W=\psi^3(xP)
    ECP2_BLS12381_dbl(&W); // W=\psi^3(2xP)
#else
//https://eprint.iacr.org/2021/1130
    ECP2_BLS12381_copy(&W,P);
    ECP2_BLS12381_frob(&W, &X);    // W=\psi(P)    
#endif

    if (ECP2_BLS12381_equals(&W,&T)) return 1;
    return 0;
}

/* Check that m is in cyclotomic sub-group */
/* Check that m!=1, conj(m)*m==1, and m.m^{p^4}=m^{p^2} */
int PAIR_BLS12381_GTcyclotomic(FP12_BLS12381 *m)
{
	FP_BLS12381 fx,fy;
	FP2_BLS12381 X;
	FP12_BLS12381 r,w;
...
int PAIR_BLS12381_GTmember(FP12_BLS12381 *m)
{
    ...
    FP_BLS12381 fx, fy;
    FP12_BLS12381 r,t;
    if (!PAIR_BLS12381_GTcyclotomic(m)) return 0;
    ...
    FP12_BLS12381_pow(&t,m,x);

#if PAIRING_FRIENDLY_BLS12381==BN_CURVE
    FP12_BLS12381_pow(&t,&t,x);
    BIG_384_29_zero(x); BIG_384_29_inc(x,6);
    FP12_BLS12381_pow(&t,&t,x);
#else
#if SIGN_OF_X_BLS12381==NEGATIVEX
    FP12_BLS12381_conj(&t,&t);
#endif
#endif

    if (FP12_BLS12381_equals(&r,&t)) return 1;
    return 0;
