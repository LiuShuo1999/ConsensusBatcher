...
void ECP2_BLS12381_toOctet(octet *W, ECP2_BLS12381 *Q, bool compress)
{
    ...
    if (alt)
    {
        FP2_BLS12381_toBytes(&(W->val[0]),&qx);
        if (!compress)
        {
            W->len=4*MODBYTES_384_29;
            FP2_BLS12381_toBytes(&(W->val[2*MODBYTES_384_29]), &qy);
        } else {
            W->val[0]|=0x80;
            if (FP2_BLS12381_islarger(&qy)==1) W->val[0]|=0x20;
            W->len=2*MODBYTES_384_29;
        }
    } else {
        FP2_BLS12381_toBytes(&(W->val[1]),&qx);
        if (!compress)
        {
            W->val[0] = 0x04;
            FP2_BLS12381_toBytes(&(W->val[2 * MODBYTES_384_29+1]), &qy);
            W->len = 4 * MODBYTES_384_29 + 1;
        } else {
            W->val[0]=0x02;
...
int ECP2_BLS12381_fromOctet(ECP2_BLS12381 *Q, octet *W)
{
    ...
    {
        W->val[0]&=0x1f;
        FP2_BLS12381_fromBytes(&qx,&(W->val[0]));
        W->val[0]=typ;
        if ((typ&0x80)==0)
        {
            FP2_BLS12381_fromBytes(&qy,&(W->val[2*MODBYTES_384_29]));
            if (ECP2_BLS12381_set(Q, &qx, &qy)) return 1;
            return 0;
        } else {
            if (!ECP2_BLS12381_setx(Q,&qx,0)) return 0;
            sgn=(typ&0x20)>>5;
            cmp=FP2_BLS12381_islarger(&(Q->y));
            if ((sgn==1 && cmp!=1) || (sgn==0 && cmp==1)) ECP2_BLS12381_neg(Q);
            return 1;
        }

    } else {
        FP2_BLS12381_fromBytes(&qx,&(W->val[1]));
        if (typ == 0x04)
        {
            FP2_BLS12381_fromBytes(&qy,&(W->val[2 * MODBYTES_384_29+1]));
            if (ECP2_BLS12381_set(Q, &qx, &qy)) return 1;
        } else {
...
void ECP2_BLS12381_rhs(FP2_BLS12381 *rhs, FP2_BLS12381 *x)
{
    ...
    BIG_384_29_rcopy(b, CURVE_B_BLS12381);
...
int ECP2_BLS12381_setx(ECP2_BLS12381 *P, FP2_BLS12381 *x, int s)
{
    ...
    if (!FP2_BLS12381_qr(&y,&h))
    {
        ECP2_BLS12381_inf(P);
        return 0;
    }
    FP2_BLS12381_sqrt(&y, &y, &h);
...
/* P*=e */
/* SU= 280 */
void ECP2_BLS12381_mul(ECP2_BLS12381 *P, BIG_384_29 e)
{
    /* fixed size windows */
    int i, nb, s, ns;
    BIG_384_29 mt, t;
...
void ECP2_BLS12381_mul4(ECP2_BLS12381 *P, ECP2_BLS12381 Q[4], BIG_384_29 u[4])
{
    int i, j, k, nb, pb, bt;
    ECP2_BLS12381 T[8], W;
    BIG_384_29 t[4], mt;
...
/*
void ECP2_BLS12381_hap2point(ECP2_BLS12381 *Q,BIG_384_29 h)
{
    BIG_384_29 one,hv;
    FP2_BLS12381 X;
    BIG_384_29_one(one);
    BIG_384_29_copy(hv,h);
    for (;;)
    {
...
{ // SSWU plus isogenies method
    ...
    FP_BLS12381 hint;
    FP2_BLS12381 ZZ,Ad,Bd,D,D2,GX1;
    ...
    FP2_BLS12381_copy(&X2,&W);
    FP2_BLS12381_mul(&X3,&T,&X2);

// x^3+Ad^2x+Bd^3
    FP2_BLS12381_sqr(&GX1,&X2);
    FP2_BLS12381_sqr(&D2,&D); FP2_BLS12381_mul(&W,&Ad,&D2); FP2_BLS12381_add(&GX1,&GX1,&W); FP2_BLS12381_norm(&GX1); FP2_BLS12381_mul(&GX1,&GX1,&X2); FP2_BLS12381_mul(&D2,&D2,&D); FP2_BLS12381_mul(&W,&Bd,&D2); FP2_BLS12381_add(&GX1,&GX1,&W); FP2_BLS12381_norm(&GX1);

    FP2_BLS12381_mul(&W,&GX1,&D);
    int qr=FP2_BLS12381_qr(&W,&hint);   // qr(ad) - only exp happens here
    FP2_BLS12381_inv(&D,&W,&hint);     // d=1/(ad)
    FP2_BLS12381_mul(&D,&D,&GX1);     // 1/d
    FP2_BLS12381_mul(&X2,&X2,&D);     // X2/=D
    FP2_BLS12381_mul(&X3,&X3,&D);     // X3/=D
    FP2_BLS12381_mul(&T,&T,H);        // t=Z.u^3
    FP2_BLS12381_sqr(&D2,&D);

// first solution - X2, W, hint, D2

    FP2_BLS12381_mul(&D,&D2,&T);     // second candidate if X3 is correct
    FP2_BLS12381_mul(&T,&W,&ZZ); 

    FP_BLS12381_rcopy(&s,CURVE_HTPC2_BLS12381);     
    FP_BLS12381_mul(&s,&s,&hint); // modify hint

    FP2_BLS12381_cmove(&X2,&X3,1-qr); 
    FP2_BLS12381_cmove(&W,&T,1-qr);
    FP2_BLS12381_cmove(&D2,&D,1-qr);
    FP_BLS12381_cmove(&hint,&s,1-qr);

    FP2_BLS12381_sqrt(&Y,&W,&hint);  // first candidate if X2 is correct
    FP2_BLS12381_mul(&Y,&Y,&D2);
...
// yden 
   ...
   if (CURVE_B_I_BLS12381==4 && SEXTIC_TWIST_BLS12381==M_TYPE && RIADZG2A_BLS12381==-1 && RIADZG2B_BLS12381==0)
    { // special case for BLS12381
        ...
        FP2_BLS12381_sqrt(&W,&A,NULL);   // sqrt(g(Z))
    }
    ...
    FP2_BLS12381_inv(&NY,&NY,NULL);     // tv3=inv0(tv1*tv2)
    ...
    FP2_BLS12381_cmove(&X3,&X2,FP2_BLS12381_qr(&W,NULL));
    ECP2_BLS12381_rhs(&W,&X1);
    FP2_BLS12381_cmove(&X3,&X1,FP2_BLS12381_qr(&W,NULL));
    ECP2_BLS12381_rhs(&W,&X3);
    FP2_BLS12381_sqrt(&Y,&W,NULL);
...
#if SEXTIC_TWIST_BLS12381==M_TYPE
    FP2_BLS12381_inv(&X, &X,NULL);
    FP2_BLS12381_norm(&X);
#endif

    BIG_384_29_rcopy(x, CURVE_Bnx_BLS12381);
