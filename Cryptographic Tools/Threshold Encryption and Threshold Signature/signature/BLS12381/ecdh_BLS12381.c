...
int ECP_BLS12381_IN_RANGE(octet* S)
{
    BIG_384_29 r,s;
...
int ECP_BLS12381_PUBLIC_KEY_VALIDATE(octet *W)
{
    ...
    {   /* Check point is not in wrong group */
        ...
        if (!BIG_384_29_isunity(k)) ECP_BLS12381_mul(&WP, k);
...
int ECP_BLS12381_SVDP_DH(octet *S, octet *WD, octet *Z, int type)
{
    BIG_384_29 r, s, wx;
    int valid;
    ECP_BLS12381 W;
    ...
    if (res == 0)
    {
        ...
        else
        {
            ...
            if (type>0)
            {
                if (type==1) ECP_BLS12381_toOctet(Z,&W,true);
                else ECP_BLS12381_toOctet(Z,&W,false);
                return res;
            }
            else
...
int ECP_BLS12381_SP_DSA(int hlen, csprng *RNG, octet *K, octet *S, octet *F, octet *C, octet *D)
{
    ...
    int blen = H.len;
    if (H.len > EGS_BLS12381) blen = EGS_BLS12381;
...
int ECP_BLS12381_VP_DSA(int hlen, octet *W, octet *F, octet *C, octet *D)
{
    ...
    int blen = H.len;
    if (blen > EGS_BLS12381) blen = EGS_BLS12381;
...
void ECP_BLS12381_ECIES_ENCRYPT(int hlen, octet *P1, octet *P2, csprng *RNG, octet *W, octet *M, int tlen, octet *V, octet *C, octet *T)
{
    ...
    if (ECP_BLS12381_SVDP_DH(&U, W, &Z, 0) != 0) return;
...
int ECP_BLS12381_ECIES_DECRYPT(int hlen, octet *P1, octet *P2, octet *V, octet *C, octet *T, octet *U, octet *M)
{
    ...
    if (ECP_BLS12381_SVDP_DH(U, V, &Z, 0) != 0) return 0;
