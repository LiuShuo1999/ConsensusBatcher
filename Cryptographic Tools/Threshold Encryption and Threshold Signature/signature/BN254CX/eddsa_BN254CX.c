...
static void dom2(bool ph,int cl,octet *DOM) {
    OCT_jstring(DOM,(char *)"SigBN254CX no BN254CX collisions");
    if (ph)
        OCT_jbyte(DOM,1,1);
    else
        OCT_jbyte(DOM,0,1);
...
static void dom4(bool ph,int cl,octet *DOM) {
    OCT_jstring(DOM,(char *)"SigBN254CX");
    if (ph)
        OCT_jbyte(DOM,1,1);
    else
        OCT_jbyte(DOM,0,1);
...
{
#if CURVE_SECURITY_BN254CX <= 128  // for ed25519?
    hash512 sh512;
    HASH512_init(&sh512);
...
static int encode_int(BIG_256_28 x,char *ei) {
    ...
    if (8*MODBYTES_256_28==MBITS_BN254CX) index=1; // extra byte needed for compression        
...
// encode point
static void encode(ECP_BN254CX *P,octet *W) {
    BIG_256_28 x,y;
    int b,index=0;
...
static void decode(octet *W,ECP_BN254CX *P) {
    BIG_256_28 X,Y;
    FP_BN254CX x,d,t,one,hint;
    int sign=0;  // LSB of x

    sign=decode_int(true,W->val,Y);
    FP_BN254CX_nres(&x,Y); FP_BN254CX_sqr(&x,&x);
    FP_BN254CX_copy(&d,&x); FP_BN254CX_one(&one);
    FP_BN254CX_sub(&x,&x,&one);
    FP_BN254CX_norm(&x);
    FP_BN254CX_rcopy(&t, CURVE_B_BN254CX);
    FP_BN254CX_mul(&d,&d,&t);
#if CURVE_A_BN254CX == 1
    FP_BN254CX_sub(&d,&d,&one);
#else
    FP_BN254CX_add(&d,&d,&one);
#endif    
    FP_BN254CX_norm(&d);
// inverse square root trick for sqrt(x/d)
    FP_BN254CX_sqr(&t,&x);
    FP_BN254CX_mul(&x,&x,&t);
    FP_BN254CX_mul(&x,&x,&d);
    if (!FP_BN254CX_qr(&x,&hint))
    {
        ECP_BN254CX_inf(P);
        return;
    }
    FP_BN254CX_sqrt(&d,&x,&hint);
    FP_BN254CX_inv(&x,&x,&hint);
    FP_BN254CX_mul(&x,&x,&d);
    FP_BN254CX_mul(&x,&x,&t);
 ...
 */
int EDDSA_BN254CX_KEY_PAIR_GENERATE(csprng *RNG,octet* D,octet *Q)
{
    BIG_256_28 r, x, y, s;
    ECP_BN254CX G;
    char digest[128];
...
int EDDSA_BN254CX_SIGNATURE(bool ph,octet *D, octet *context,octet *M,octet *SIG)
{
    DBIG_256_28 dr;
    ...
    EDDSA_BN254CX_KEY_PAIR_GENERATE(NULL,D,&Q);

    BIG_256_28_rcopy(q, CURVE_Order_BN254CX);
    ECP_BN254CX_generator(&R);
...
bool EDDSA_BN254CX_VERIFY(bool ph,octet *Q,octet *context,octet *M,octet *SIG) 
{
    DBIG_256_28 du;
    BIG_256_28 q,t,su;
    ECP_BN254CX R,QD,G;
    ...
    decode(SIG,&R);
    if (ECP_BN254CX_isinf(&R)) return false;
    decode_int(false,&SIG->val[b],t);
    if (BIG_256_28_comp(t,q)>=0) return false;
    decode(Q,&QD);
    if (ECP_BN254CX_isinf(&QD)) return false;
    ...
    for (int i=0;i<lg;i++)
    { // use cofactor 2^c
        ECP_BN254CX_dbl(&G); ECP_BN254CX_dbl(&QD); ECP_BN254CX_dbl(&R);
    }

    ECP_BN254CX_neg(&QD);
    ECP_BN254CX_mul2(&G,&QD,t,su);

    if (!ECP_BN254CX_equals(&G,&R)) return false;
