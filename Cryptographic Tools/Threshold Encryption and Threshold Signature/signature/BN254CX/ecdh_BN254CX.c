...
static void RFC7748(BIG_256_28 r)
{
    int c,lg=0;
    BIG_256_28 t;
    c=CURVE_Cof_I_BN254CX;
    while (c!=1)
    {
        lg++;
        c/=2;
    }
    int n=8*EGS_BN254CX-lg+1;
...
int ECP_BN254CX_IN_RANGE(octet* S)
{
    BIG_256_28 r,s;
...
int ECP_BN254CX_KEY_PAIR_GENERATE(csprng *RNG, octet* S, octet *W)
{
    BIG_256_28 r, s;
    ECP_BN254CX G;
    int res = 0;

    ECP_BN254CX_generator(&G);
    BIG_256_28_rcopy(r, CURVE_Order_BN254CX);
    ...
    ECP_BN254CX_clmul(&G, s, r);
    ECP_BN254CX_toOctet(W, &G, false); /* To use point compression on public keys, change to true */

    return res;
}

/* Validate public key */
int ECP_BN254CX_PUBLIC_KEY_VALIDATE(octet *W)
{
    BIG_256_28 q, r, k;
    ECP_BN254CX WP;
    int valid, nb;
    ...
    valid = ECP_BN254CX_fromOctet(&WP, W);
    ...
    {   /* Check point is not in wrong group */
        ...
        if (!BIG_256_28_isunity(k)) ECP_BN254CX_mul(&WP, k);
        if (ECP_BN254CX_isinf(&WP)) res = ECDH_INVALID_PUBLIC_KEY;
    }
...
int ECP_BN254CX_SVDP_DH(octet *S, octet *WD, octet *Z, int type)
{
    BIG_256_28 r, s, wx;
    int valid;
    ECP_BN254CX W;
    ...
    BIG_256_28_fromBytes(s, S->val);
    valid = ECP_BN254CX_fromOctet(&W, WD);

    if (!valid) res = ECDH_ERROR;
    if (res == 0)
    {
        BIG_256_28_rcopy(r, CURVE_Order_BN254CX);
        ECP_BN254CX_clmul(&W, s, r);
        if (ECP_BN254CX_isinf(&W)) res = ECDH_ERROR;
        else
        {
#if CURVETYPE_BN254CX!=MONTGOMERY
            if (type>0)
            {
                if (type==1) ECP_BN254CX_toOctet(Z,&W,true);
                else ECP_BN254CX_toOctet(Z,&W,false);
                return res;
            }
            else
                ECP_BN254CX_get(wx, wx, &W);
#else
            ECP_BN254CX_get(wx, &W);
#endif
            Z->len = EFS_BN254CX;
...
int ECP_BN254CX_SP_DSA(int hlen, csprng *RNG, octet *K, octet *S, octet *F, octet *C, octet *D)
{
    char h[128];
    ...
    int blen = H.len;
    if (H.len > EGS_BN254CX) blen = EGS_BN254CX;
    ...
    if (RNG != NULL)
    {
        do
        {
            ...
            ECP_BN254CX_copy(&V, &G);
            ECP_BN254CX_clmul(&V, u, r);

            ECP_BN254CX_get(vx, vx, &V);
...
int ECP_BN254CX_VP_DSA(int hlen, octet *W, octet *F, octet *C, octet *D)
{
    char h[128];
    octet H = {0, sizeof(h), h};
    ...
    int blen = H.len;
    if (blen > EGS_BN254CX) blen = EGS_BN254CX;
    ...
    if (res == 0)
    {
        ...
        {
            ECP_BN254CX_mul2(&WP, &G, h2, f);
            if (ECP_BN254CX_isinf(&WP)) res = ECDH_ERROR;
            else
            {
                ECP_BN254CX_get(d, d, &WP);
                BIG_256_28_mod(d, r);
...
void ECP_BN254CX_ECIES_ENCRYPT(int hlen, octet *P1, octet *P2, csprng *RNG, octet *W, octet *M, int tlen, octet *V, octet *C, octet *T)
{

    int i, len;
    char z[EFS_BN254CX], vz[3 * EFS_BN254CX + 1], k[2 * AESKEY_BN254CX], k1[AESKEY_BN254CX], k2[AESKEY_BN254CX], l2[8], u[EFS_BN254CX];
    ...
    if (ECP_BN254CX_KEY_PAIR_GENERATE(RNG, &U, V) != 0) return;
    if (ECP_BN254CX_SVDP_DH(&U, W, &Z, 0) != 0) return;

    OCT_copy(&VZ, V);
    OCT_joctet(&VZ, &Z);

    KDF2(MC_SHA2, hlen, &K, 2 * AESKEY_BN254CX, &VZ, P1);

    K1.len = K2.len = AESKEY_BN254CX;
    for (i = 0; i < AESKEY_BN254CX; i++)
    {
        K1.val[i] = K.val[i];
        K2.val[i] = K.val[AESKEY_BN254CX + i];
    }
...
int ECP_BN254CX_ECIES_DECRYPT(int hlen, octet *P1, octet *P2, octet *V, octet *C, octet *T, octet *U, octet *M)
{

    int i, len;
    char z[EFS_BN254CX], vz[3 * EFS_BN254CX + 1], k[2 * AESKEY_BN254CX], k1[AESKEY_BN254CX], k2[AESKEY_BN254CX], l2[8], tag[32];
    octet Z = {0, sizeof(z), z};
    octet VZ = {0, sizeof(vz), vz};
    octet K = {0, sizeof(k), k};
    octet K1 = {0, sizeof(k1), k1};
    ...
    if (ECP_BN254CX_SVDP_DH(U, V, &Z, 0) != 0) return 0;
