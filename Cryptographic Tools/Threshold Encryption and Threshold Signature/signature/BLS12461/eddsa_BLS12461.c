...
static void dom2(bool ph,int cl,octet *DOM) {
    OCT_jstring(DOM,(char *)"SigBLS12461 no BLS12461 collisions");
    if (ph)
        OCT_jbyte(DOM,1,1);
    else
        OCT_jbyte(DOM,0,1);
...
static void dom4(bool ph,int cl,octet *DOM) {
    OCT_jstring(DOM,(char *)"SigBLS12461");
    if (ph)
        OCT_jbyte(DOM,1,1);
    else
        OCT_jbyte(DOM,0,1);
...
{
#if CURVE_SECURITY_BLS12461 <= 128  // for ed25519?
    hash512 sh512;
    HASH512_init(&sh512);
...
static int encode_int(BIG_464_28 x,char *ei) {
    ...
    if (8*MODBYTES_464_28==MBITS_BLS12461) index=1; // extra byte needed for compression        
...
// encode point
static void encode(ECP_BLS12461 *P,octet *W) {
    BIG_464_28 x,y;
    int b,index=0;
...
static void decode(octet *W,ECP_BLS12461 *P) {
    BIG_464_28 X,Y;
    FP_BLS12461 x,d,t,one,hint;
    int sign=0;  // LSB of x

    sign=decode_int(true,W->val,Y);
    FP_BLS12461_nres(&x,Y); FP_BLS12461_sqr(&x,&x);
    FP_BLS12461_copy(&d,&x); FP_BLS12461_one(&one);
    FP_BLS12461_sub(&x,&x,&one);
    FP_BLS12461_norm(&x);
    FP_BLS12461_rcopy(&t, CURVE_B_BLS12461);
    FP_BLS12461_mul(&d,&d,&t);
#if CURVE_A_BLS12461 == 1
    FP_BLS12461_sub(&d,&d,&one);
#else
    FP_BLS12461_add(&d,&d,&one);
#endif    
    FP_BLS12461_norm(&d);
// inverse square root trick for sqrt(x/d)
    FP_BLS12461_sqr(&t,&x);
    FP_BLS12461_mul(&x,&x,&t);
    FP_BLS12461_mul(&x,&x,&d);
    if (!FP_BLS12461_qr(&x,&hint))
    {
        ECP_BLS12461_inf(P);
        return;
    }
    FP_BLS12461_sqrt(&d,&x,&hint);
    FP_BLS12461_inv(&x,&x,&hint);
    FP_BLS12461_mul(&x,&x,&d);
    FP_BLS12461_mul(&x,&x,&t);

    FP_BLS12461_reduce(&x);
    FP_BLS12461_redc(X,&x);
 ...
 */
int EDDSA_BLS12461_KEY_PAIR_GENERATE(csprng *RNG,octet* D,octet *Q)
{
    BIG_464_28 r, x, y, s;
    ECP_BLS12461 G;
    char digest[128];
...
int EDDSA_BLS12461_SIGNATURE(bool ph,octet *D, octet *context,octet *M,octet *SIG)
{
    DBIG_464_28 dr;
    ...
    EDDSA_BLS12461_KEY_PAIR_GENERATE(NULL,D,&Q);

    BIG_464_28_rcopy(q, CURVE_Order_BLS12461);
    ECP_BLS12461_generator(&R);
...
bool EDDSA_BLS12461_VERIFY(bool ph,octet *Q,octet *context,octet *M,octet *SIG) 
{
    DBIG_464_28 du;
    BIG_464_28 q,t,su;
    ECP_BLS12461 R,QD,G;
    ...
    decode(SIG,&R);
    if (ECP_BLS12461_isinf(&R)) return false;
    decode_int(false,&SIG->val[b],t);
    if (BIG_464_28_comp(t,q)>=0) return false;
    decode(Q,&QD);
    if (ECP_BLS12461_isinf(&QD)) return false;
    ...
    for (int i=0;i<lg;i++)
    { // use cofactor 2^c
        ECP_BLS12461_dbl(&G); ECP_BLS12461_dbl(&QD); ECP_BLS12461_dbl(&R);
    }

    ECP_BLS12461_neg(&QD);
    ECP_BLS12461_mul2(&G,&QD,t,su);

    if (!ECP_BLS12461_equals(&G,&R)) return false;
