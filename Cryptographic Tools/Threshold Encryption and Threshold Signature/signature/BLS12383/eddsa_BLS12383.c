...
static void dom2(bool ph,int cl,octet *DOM) {
    OCT_jstring(DOM,(char *)"SigBLS12383 no BLS12383 collisions");
    if (ph)
        OCT_jbyte(DOM,1,1);
    else
        OCT_jbyte(DOM,0,1);
...
static void dom4(bool ph,int cl,octet *DOM) {
    OCT_jstring(DOM,(char *)"SigBLS12383");
    if (ph)
        OCT_jbyte(DOM,1,1);
    else
        OCT_jbyte(DOM,0,1);
...
{
#if CURVE_SECURITY_BLS12383 <= 128  // for ed25519?
    hash512 sh512;
    HASH512_init(&sh512);
...
static int encode_int(BIG_384_29 x,char *ei) {
    ...
    if (8*MODBYTES_384_29==MBITS_BLS12383) index=1; // extra byte needed for compression        
...
// encode point
static void encode(ECP_BLS12383 *P,octet *W) {
    BIG_384_29 x,y;
    int b,index=0;
...
static void decode(octet *W,ECP_BLS12383 *P) {
    BIG_384_29 X,Y;
    FP_BLS12383 x,d,t,one,hint;
    int sign=0;  // LSB of x

    sign=decode_int(true,W->val,Y);
    FP_BLS12383_nres(&x,Y); FP_BLS12383_sqr(&x,&x);
    FP_BLS12383_copy(&d,&x); FP_BLS12383_one(&one);
    FP_BLS12383_sub(&x,&x,&one);
    FP_BLS12383_norm(&x);
    FP_BLS12383_rcopy(&t, CURVE_B_BLS12383);
    FP_BLS12383_mul(&d,&d,&t);
#if CURVE_A_BLS12383 == 1
    FP_BLS12383_sub(&d,&d,&one);
#else
    FP_BLS12383_add(&d,&d,&one);
#endif    
    FP_BLS12383_norm(&d);
// inverse square root trick for sqrt(x/d)
    FP_BLS12383_sqr(&t,&x);
    FP_BLS12383_mul(&x,&x,&t);
    FP_BLS12383_mul(&x,&x,&d);
    if (!FP_BLS12383_qr(&x,&hint))
    {
        ECP_BLS12383_inf(P);
        return;
    }
    FP_BLS12383_sqrt(&d,&x,&hint);
    FP_BLS12383_inv(&x,&x,&hint);
    FP_BLS12383_mul(&x,&x,&d);
    FP_BLS12383_mul(&x,&x,&t);

    FP_BLS12383_reduce(&x);
    FP_BLS12383_redc(X,&x);
 ...
 */
int EDDSA_BLS12383_KEY_PAIR_GENERATE(csprng *RNG,octet* D,octet *Q)
{
    BIG_384_29 r, x, y, s;
    ECP_BLS12383 G;
    char digest[128];
...
int EDDSA_BLS12383_SIGNATURE(bool ph,octet *D, octet *context,octet *M,octet *SIG)
{
    DBIG_384_29 dr;
    ...
    EDDSA_BLS12383_KEY_PAIR_GENERATE(NULL,D,&Q);

    BIG_384_29_rcopy(q, CURVE_Order_BLS12383);
    ECP_BLS12383_generator(&R);
...
bool EDDSA_BLS12383_VERIFY(bool ph,octet *Q,octet *context,octet *M,octet *SIG) 
{
    DBIG_384_29 du;
    BIG_384_29 q,t,su;
    ECP_BLS12383 R,QD,G;
    ...
    decode(SIG,&R);
    if (ECP_BLS12383_isinf(&R)) return false;
    decode_int(false,&SIG->val[b],t);
    if (BIG_384_29_comp(t,q)>=0) return false;
    decode(Q,&QD);
    if (ECP_BLS12383_isinf(&QD)) return false;
    ...
    for (int i=0;i<lg;i++)
    { // use cofactor 2^c
        ECP_BLS12383_dbl(&G); ECP_BLS12383_dbl(&QD); ECP_BLS12383_dbl(&R);
    }

    ECP_BLS12383_neg(&QD);
    ECP_BLS12383_mul2(&G,&QD,t,su);

    if (!ECP_BLS12383_equals(&G,&R)) return false;
