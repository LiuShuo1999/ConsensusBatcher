...
static void dom2(bool ph,int cl,octet *DOM) {
    OCT_jstring(DOM,(char *)"SigBLS48556 no BLS48556 collisions");
    if (ph)
        OCT_jbyte(DOM,1,1);
    else
        OCT_jbyte(DOM,0,1);
...
static void dom4(bool ph,int cl,octet *DOM) {
    OCT_jstring(DOM,(char *)"SigBLS48556");
    if (ph)
        OCT_jbyte(DOM,1,1);
    else
        OCT_jbyte(DOM,0,1);
...
{
#if CURVE_SECURITY_BLS48556 <= 128  // for ed25519?
    hash512 sh512;
    HASH512_init(&sh512);
...
static int H2(bool ph,octet *context,octet *R,octet *Q,octet *M,DBIG_560_29 dr)
{
    char h[128];
    int b=Q->len;
    char dom[64];
...
#endif
    reverse(2*b,h);
    BIG_560_29_dfromBytesLen(dr,h,2*b);
    return 2*b;
}

static void getR(bool ph,int b,char *digest,octet *context,octet *M,DBIG_560_29 dr) {
    char h[128];
    char dom[64];
    octet DOM={0,sizeof(dom),dom};
    int cl;
...
static int encode_int(BIG_560_29 x,char *ei) {
    int b,index=0;

    if (8*MODBYTES_560_29==MBITS_BLS48556) index=1; // extra byte needed for compression        
    b=MODBYTES_560_29+index;

    ei[0]=0;
    BIG_560_29_toBytes(&ei[index],x);
    reverse(b,ei);
    return b;
}

// encode point
static void encode(ECP_BLS48556 *P,octet *W) {
    BIG_560_29 x,y;
    int b,index=0;
    ...
    W->val[b-1]|=BIG_560_29_parity(x)<<7;
    W->len=b;
}

// decode integer (little endian)
static int decode_int(bool strip_sign,char *ei,BIG_560_29 x) {
    int b,index=0;
    int sign=0;
    char r[MODBYTES_560_29+1];
    ...
    if (r[0]==0) // remove leading zero
        BIG_560_29_fromBytesLen(x,&r[1],b-1);
    else
        BIG_560_29_fromBytesLen(x,&r[0],b);
    return sign;
}

// decode compressed point
static void decode(octet *W,ECP_BLS48556 *P) {
    BIG_560_29 X,Y;
    FP_BLS48556 x,d,t,one,hint;
    int sign=0;  // LSB of x

    sign=decode_int(true,W->val,Y);
    FP_BLS48556_nres(&x,Y); FP_BLS48556_sqr(&x,&x);
    FP_BLS48556_copy(&d,&x); FP_BLS48556_one(&one);
    FP_BLS48556_sub(&x,&x,&one);
    FP_BLS48556_norm(&x);
    FP_BLS48556_rcopy(&t, CURVE_B_BLS48556);
    FP_BLS48556_mul(&d,&d,&t);
#if CURVE_A_BLS48556 == 1
    FP_BLS48556_sub(&d,&d,&one);
#else
    FP_BLS48556_add(&d,&d,&one);
#endif    
    FP_BLS48556_norm(&d);
// inverse square root trick for sqrt(x/d)
    FP_BLS48556_sqr(&t,&x);
    FP_BLS48556_mul(&x,&x,&t);
    FP_BLS48556_mul(&x,&x,&d);
    if (!FP_BLS48556_qr(&x,&hint))
    {
        ECP_BLS48556_inf(P);
        return;
    }
    FP_BLS48556_sqrt(&d,&x,&hint);
    FP_BLS48556_inv(&x,&x,&hint);
    FP_BLS48556_mul(&x,&x,&d);
    FP_BLS48556_mul(&x,&x,&t);

    FP_BLS48556_reduce(&x);
    FP_BLS48556_redc(X,&x);
    if (BIG_560_29_parity(X)!=sign)
        FP_BLS48556_neg(&x,&x);
    FP_BLS48556_norm(&x);
    FP_BLS48556_redc(X,&x);
 ...
 */
int EDDSA_BLS48556_KEY_PAIR_GENERATE(csprng *RNG,octet* D,octet *Q)
{
    BIG_560_29 r, x, y, s;
    ECP_BLS48556 G;
    char digest[128];
...
// reverse bytes for little endian
    reverse(b,digest);
    BIG_560_29_fromBytes(s,&digest[index]);
    RFC7748(s);
    ECP_BLS48556_mul(&G, s);
...
int EDDSA_BLS48556_SIGNATURE(bool ph,octet *D, octet *context,octet *M,octet *SIG)
{
    DBIG_560_29 dr;
    BIG_560_29 s,sr,sd,q;
    ECP_BLS48556 R;
    char digest[128];
    char pq[MODBYTES_560_29+1];
    ...
    EDDSA_BLS48556_KEY_PAIR_GENERATE(NULL,D,&Q);

    BIG_560_29_rcopy(q, CURVE_Order_BLS48556);
    ECP_BLS48556_generator(&R);
    ...
    if (res==EDDSA_OK)
    {
        ...
        BIG_560_29_dmod(sr,dr,q);
        ECP_BLS48556_mul(&R,sr);
        encode(&R,SIG);
// reverse bytes for little endian        
        reverse(b,digest);
        BIG_560_29_fromBytes(s,&digest[index]);
        ...
        BIG_560_29_dmod(sd,dr,q);
        BIG_560_29_modmul(s,s,sd,q);
        BIG_560_29_modadd(s,s,sr,q);
...
bool EDDSA_BLS48556_VERIFY(bool ph,octet *Q,octet *context,octet *M,octet *SIG) 
{
    DBIG_560_29 du;
    BIG_560_29 q,t,su;
    ECP_BLS48556 R,QD,G;
    ...
    decode(SIG,&R);
    if (ECP_BLS48556_isinf(&R)) return false;
    decode_int(false,&SIG->val[b],t);
    if (BIG_560_29_comp(t,q)>=0) return false;
    decode(Q,&QD);
    if (ECP_BLS48556_isinf(&QD)) return false;

    H2(ph,context,SIG,Q,M,du);
    BIG_560_29_dmod(su,du,q);
    ECP_BLS48556_generator(&G);
    for (int i=0;i<lg;i++)
    { // use cofactor 2^c
        ECP_BLS48556_dbl(&G); ECP_BLS48556_dbl(&QD); ECP_BLS48556_dbl(&R);
    }

    ECP_BLS48556_neg(&QD);
    ECP_BLS48556_mul2(&G,&QD,t,su);

    if (!ECP_BLS48556_equals(&G,&R)) return false;
