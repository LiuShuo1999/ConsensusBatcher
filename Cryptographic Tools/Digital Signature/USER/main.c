...
//任务优先级
#define Digital_Signature_TASK_PRIO		2
//任务堆栈大小	
#define Digital_Signature_STK_SIZE 		4096  
//任务句柄
TaskHandle_t Digital_Signature_Task_Handler;
//任务函数
void Digital_Signature(void *pvParameters);
...
void start_task(void *pvParameters)
{
    ...
    //创建LED0任务
    xTaskCreate((TaskFunction_t )Digital_Signature,     	
                (const char*    )"Digital_Signature",   	
                (uint16_t       )Digital_Signature_STK_SIZE, 
                (void*          )NULL,				
                (UBaseType_t    )Digital_Signature_TASK_PRIO,	
                (TaskHandle_t*  )&Digital_Signature_Task_Handler);   
//    //创建LED1任务
...
//	TIM2->CNT = 0;TIM2_Exceed_Times=0;
//	delay_ms(200);
//	btim_tim2_enable(0);
//	while(1);
	
    uint8_t privatex[32] = {0};
    uint8_t publicx[64] = {0};
    uint8_t hash[32] = {0};
    uint8_t sig[64] = {0};
	...
	u8 print_key = 1;
	uint8_t privatey[13][32] = {0};
    uint8_t publicy[13][64] = {0};
	u8 print_num = 3;
	if(print_key){
		for(int i=0;i<13;i++){
			if (!uECC_make_key(publicy[i], privatey[i], uECC_secp160r1())) {
		...
		for(int i=0;i<print_num;i++){
			sprintf((char*)Send_Data_buff,"*");LoRa_SendData(Send_Data_buff);
			for (int m = 0; m < 32; ++m) {
				sprintf((char*)Send_Data_buff,"%02x",privatey[i][m]);LoRa_SendData(Send_Data_buff);
				delay_ms(200);
			}
		}
		...
		for(int i=0;i<print_num;i++){
			sprintf((char*)Send_Data_buff,"*");LoRa_SendData(Send_Data_buff);
			for (int m = 0; m < 64; ++m) {
				sprintf((char*)Send_Data_buff,"%02x",publicy[i][m]);LoRa_SendData(Send_Data_buff);
				delay_ms(200);
			}
		}
    ...
    u32 key_time[6][100];
	u32 sign_time[6][100];
	u32 verify_time[6][100];
    for (int c = 0; c < num_curves; ++c) {
        for (int i = 0; i < run_times; ++i) {
			
			for(int j=0;j<32;j++)hash[j] = RNG_Get_RandomNum();
			for(int j=0;j<64;j++)sig[j] = 0x00;
			
			TIM2->CNT = 0;TIM2_Exceed_Times=0;
            if (!uECC_make_key(publicx, privatex, curves[c])) {
                printf("uECC_make_key() failed\n");
                while(1);
				//return 1;
            }
			key_time[c][i] = TIM2_Exceed_Times * 9000 + TIM2->CNT;
			
			TIM2->CNT = 0;TIM2_Exceed_Times=0;
            if (!uECC_sign(privatex, hash, 32, sig, curves[c])) {
                printf("uECC_sign() failed\n");
                while(1);
				//return 1;
            }
			sign_time[c][i] = TIM2_Exceed_Times * 9000 + TIM2->CNT;
			
			TIM2->CNT = 0;TIM2_Exceed_Times=0;
            if (!uECC_verify(publicx, hash, 32, sig, curves[c])) {
                printf("uECC_verify() failed\n");
                while(1);
				//return 1;
            }
			verify_time[c][i] = TIM2_Exceed_Times * 9000 + TIM2->CNT;
        }
	...
	u32 slot = 200;
	for (int c = 0; c < num_curves; ++c) {
        sprintf((char*)Send_Data_buff,"cuve %d",c);LoRa_SendData(Send_Data_buff);
		sprintf((char*)Send_Data_buff,"key time:");LoRa_SendData(Send_Data_buff);
		for (int i = 0; i < run_times; ++i) {
			sprintf((char*)Send_Data_buff,"%d",key_time[c][i]);LoRa_SendData(Send_Data_buff);
			delay_ms(slot);
		}
		sprintf((char*)Send_Data_buff,"sign time:");LoRa_SendData(Send_Data_buff);
		for (int i = 0; i < run_times; ++i) {
			sprintf((char*)Send_Data_buff,"%d",sign_time[c][i]);LoRa_SendData(Send_Data_buff);
			delay_ms(slot);
		}
		sprintf((char*)Send_Data_buff,"verify time:");LoRa_SendData(Send_Data_buff);
		for (int i = 0; i < run_times; ++i) {
			sprintf((char*)Send_Data_buff,"%d",verify_time[c][i]);LoRa_SendData(Send_Data_buff);
			delay_ms(slot);
		}
    }
}
