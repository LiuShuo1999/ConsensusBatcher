...
#include <stdlib.h>
#include <string.h>
#include "hpke_BLS48581.h"

#define GROUP EGS_BLS48581
#define POINT (2*EFS_BLS48581+1)
#define MAX_HASH HASH_TYPE_BLS48581
...
static void LabeledExtract(octet *PRK,octet *SALT,octet *SUITE_ID,char *label,octet *IKM)
{
    ...
    if (IKM!=NULL)
        OCT_joctet(&LIKM,IKM);
    HKDF_Extract(MC_SHA2,HASH_TYPE_BLS48581,PRK,SALT,&LIKM);
}
...
static void LabeledExpand(octet *OKM,octet *PRK,octet *SUITE_ID,char *label,octet *INFO,int L)
{
    ...
    HKDF_Expand(MC_SHA2,HASH_TYPE_BLS48581,OKM,L,PRK,&LINFO);
}
...
static void ExtractAndExpand(int config_id,octet *OKM,octet *DH,octet *CONTEXT)
{
    ...
    LabeledExpand(OKM, &PRK,&SUITE_ID,(char *)"shared_secret",CONTEXT,HASH_TYPE_BLS48581);  
}

int DeriveKeyPair_BLS48581(int config_id,octet *SK,octet *PK,octet *SEED)
{
    int counter=0;
    char prk[MAX_HASH];
    octet PRK={0,sizeof(prk),prk};
    ...
    { // RFC7748
        ...
        OCT_clear(SK);
        while (!ECP_BLS48581_IN_RANGE(SK) && counter<256)
        {
            OCT_empty(&INFO);
    ...
    ECP_BLS48581_KEY_PAIR_GENERATE(NULL, SK, PK);
    if (kem==32 || kem==33)
        OCT_reverse(PK);
...
void HPKE_BLS48581_Encap(int config_id,octet *skE,octet *Z,octet *pkE,octet *pkR)
{
    int res,kem;
    char dh[POINT];
    ...
    {
        OCT_reverse(pkR);
        res=ECP_BLS48581_SVDP_DH(skE, pkR, &DH, 0);
        OCT_reverse(pkR);
        OCT_reverse(&DH);
    } else {
        res=ECP_BLS48581_SVDP_DH(skE, pkR, &DH, 0);
...
void HPKE_BLS48581_Decap(int config_id,octet *skR,octet *Z,octet *pkE,octet *pkR)
{
    int res,kem;
    char dh[POINT];
    octet DH={0,sizeof(dh),dh};
    ...
    {
        OCT_reverse(pkE);
        ECP_BLS48581_SVDP_DH(skR, pkE, &DH, 0);
        OCT_reverse(pkE);
        OCT_reverse(&DH);
    } else {
        ECP_BLS48581_SVDP_DH(skR, pkE, &DH, 0);
...
void HPKE_BLS48581_AuthEncap(int config_id,octet *skE,octet *skS,octet *Z,octet *pkE,octet *pkR,octet *pkS)
{
    int res,kem;
    char dh[2*POINT];
    octet DH={0,sizeof(dh),dh};
    ...
    {
        OCT_reverse(pkR);
        ECP_BLS48581_SVDP_DH(skE, pkR, &DH,0);
        ECP_BLS48581_SVDP_DH(skS, pkR, &DH1,0);
        OCT_reverse(pkR);
        OCT_reverse(&DH);
        OCT_reverse(&DH1);
    } else {
...
void HPKE_BLS48581_AuthDecap(int config_id,octet *skR,octet *Z,octet *pkE,octet *pkR,octet *pkS)
{
    int res,kem;
    char dh[2*POINT];
    octet DH={0,sizeof(dh),dh};
    ...
    if (kem==32 || kem==33) {
        ...
        OCT_reverse(pkS);
        ECP_BLS48581_SVDP_DH(skR, pkE, &DH,0);
        ECP_BLS48581_SVDP_DH(skR, pkS, &DH1,0);
        OCT_reverse(pkE);
        OCT_reverse(pkS);
        OCT_reverse(&DH);
        OCT_reverse(&DH1);
...
void HPKE_BLS48581_KeySchedule(int config_id,octet *key,octet *nonce,octet *exp_secret,int mode,octet *Z,octet *info,octet *psk,octet *pskID)
{
    ...
    LabeledExpand(key,&secret,&SUITE_ID,(char *)"key",&CONTEXT,AESKEY_BLS48581);
    LabeledExpand(nonce,&secret,&SUITE_ID,(char *)"base_nonce",&CONTEXT,12);
    if (exp_secret!=NULL)
        LabeledExpand(exp_secret,&secret,&SUITE_ID,(char *)"exp",&CONTEXT,HASH_TYPE_BLS48581);
}
