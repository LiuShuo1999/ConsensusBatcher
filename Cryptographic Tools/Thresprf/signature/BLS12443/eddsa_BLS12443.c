...
static void dom2(bool ph,int cl,octet *DOM) {
    OCT_jstring(DOM,(char *)"SigBLS12443 no BLS12443 collisions");
    if (ph)
        OCT_jbyte(DOM,1,1);
    else
        OCT_jbyte(DOM,0,1);
...
static void dom4(bool ph,int cl,octet *DOM) {
    OCT_jstring(DOM,(char *)"SigBLS12443");
    if (ph)
        OCT_jbyte(DOM,1,1);
    else
        OCT_jbyte(DOM,0,1);
...
{
#if CURVE_SECURITY_BLS12443 <= 128  // for ed25519?
    hash512 sh512;
    HASH512_init(&sh512);
...
static int H2(bool ph,octet *context,octet *R,octet *Q,octet *M,DBIG_448_29 dr)
{
    char h[128];
    int b=Q->len;
    char dom[64];
...
#endif
    reverse(2*b,h);
    BIG_448_29_dfromBytesLen(dr,h,2*b);
    return 2*b;
}

static void getR(bool ph,int b,char *digest,octet *context,octet *M,DBIG_448_29 dr) {
    char h[128];
    char dom[64];
    octet DOM={0,sizeof(dom),dom};
    int cl;
...
static int encode_int(BIG_448_29 x,char *ei) {
    int b,index=0;

    if (8*MODBYTES_448_29==MBITS_BLS12443) index=1; // extra byte needed for compression        
    b=MODBYTES_448_29+index;

    ei[0]=0;
    BIG_448_29_toBytes(&ei[index],x);
    reverse(b,ei);
    return b;
}

// encode point
static void encode(ECP_BLS12443 *P,octet *W) {
    BIG_448_29 x,y;
    int b,index=0;
    ...
    W->val[b-1]|=BIG_448_29_parity(x)<<7;
    W->len=b;
}

// decode integer (little endian)
static int decode_int(bool strip_sign,char *ei,BIG_448_29 x) {
    int b,index=0;
    int sign=0;
    char r[MODBYTES_448_29+1];
    ...
    if (r[0]==0) // remove leading zero
        BIG_448_29_fromBytesLen(x,&r[1],b-1);
    else
        BIG_448_29_fromBytesLen(x,&r[0],b);
    return sign;
}

// decode compressed point
static void decode(octet *W,ECP_BLS12443 *P) {
    BIG_448_29 X,Y;
    FP_BLS12443 x,d,t,one,hint;
    int sign=0;  // LSB of x

    sign=decode_int(true,W->val,Y);
    FP_BLS12443_nres(&x,Y); FP_BLS12443_sqr(&x,&x);
    FP_BLS12443_copy(&d,&x); FP_BLS12443_one(&one);
    FP_BLS12443_sub(&x,&x,&one);
    FP_BLS12443_norm(&x);
    FP_BLS12443_rcopy(&t, CURVE_B_BLS12443);
    FP_BLS12443_mul(&d,&d,&t);
#if CURVE_A_BLS12443 == 1
    FP_BLS12443_sub(&d,&d,&one);
#else
    FP_BLS12443_add(&d,&d,&one);
#endif    
    FP_BLS12443_norm(&d);
// inverse square root trick for sqrt(x/d)
    FP_BLS12443_sqr(&t,&x);
    FP_BLS12443_mul(&x,&x,&t);
    FP_BLS12443_mul(&x,&x,&d);
    if (!FP_BLS12443_qr(&x,&hint))
    {
        ECP_BLS12443_inf(P);
        return;
    }
    FP_BLS12443_sqrt(&d,&x,&hint);
    FP_BLS12443_inv(&x,&x,&hint);
    FP_BLS12443_mul(&x,&x,&d);
    FP_BLS12443_mul(&x,&x,&t);

    FP_BLS12443_reduce(&x);
    FP_BLS12443_redc(X,&x);
    if (BIG_448_29_parity(X)!=sign)
        FP_BLS12443_neg(&x,&x);
    FP_BLS12443_norm(&x);
    FP_BLS12443_redc(X,&x);
 ...
 */
int EDDSA_BLS12443_KEY_PAIR_GENERATE(csprng *RNG,octet* D,octet *Q)
{
    BIG_448_29 r, x, y, s;
    ECP_BLS12443 G;
    char digest[128];
...
// reverse bytes for little endian
    reverse(b,digest);
    BIG_448_29_fromBytes(s,&digest[index]);
    RFC7748(s);
    ECP_BLS12443_mul(&G, s);
...
int EDDSA_BLS12443_SIGNATURE(bool ph,octet *D, octet *context,octet *M,octet *SIG)
{
    DBIG_448_29 dr;
    BIG_448_29 s,sr,sd,q;
    ECP_BLS12443 R;
    char digest[128];
    char pq[MODBYTES_448_29+1];
    ...
    EDDSA_BLS12443_KEY_PAIR_GENERATE(NULL,D,&Q);

    BIG_448_29_rcopy(q, CURVE_Order_BLS12443);
    ECP_BLS12443_generator(&R);
    ...
    if (res==EDDSA_OK)
    {
        ...
        BIG_448_29_dmod(sr,dr,q);
        ECP_BLS12443_mul(&R,sr);
        encode(&R,SIG);
// reverse bytes for little endian        
        reverse(b,digest);
        BIG_448_29_fromBytes(s,&digest[index]);
        ...
        BIG_448_29_dmod(sd,dr,q);
        BIG_448_29_modmul(s,s,sd,q);
        BIG_448_29_modadd(s,s,sr,q);
...
bool EDDSA_BLS12443_VERIFY(bool ph,octet *Q,octet *context,octet *M,octet *SIG) 
{
    DBIG_448_29 du;
    BIG_448_29 q,t,su;
    ECP_BLS12443 R,QD,G;
    ...
    decode(SIG,&R);
    if (ECP_BLS12443_isinf(&R)) return false;
    decode_int(false,&SIG->val[b],t);
    if (BIG_448_29_comp(t,q)>=0) return false;
    decode(Q,&QD);
    if (ECP_BLS12443_isinf(&QD)) return false;

    H2(ph,context,SIG,Q,M,du);
    BIG_448_29_dmod(su,du,q);
    ECP_BLS12443_generator(&G);
    for (int i=0;i<lg;i++)
    { // use cofactor 2^c
        ECP_BLS12443_dbl(&G); ECP_BLS12443_dbl(&QD); ECP_BLS12443_dbl(&R);
    }

    ECP_BLS12443_neg(&QD);
    ECP_BLS12443_mul2(&G,&QD,t,su);

    if (!ECP_BLS12443_equals(&G,&R)) return false;
