...
#include <stdlib.h>
#include <string.h>
#include "hpke_FP512BN.h"

#define GROUP EGS_FP512BN
#define POINT (2*EFS_FP512BN+1)
#define MAX_HASH HASH_TYPE_FP512BN
...
static void LabeledExtract(octet *PRK,octet *SALT,octet *SUITE_ID,char *label,octet *IKM)
{
    ...
    if (IKM!=NULL)
        OCT_joctet(&LIKM,IKM);
    HKDF_Extract(MC_SHA2,HASH_TYPE_FP512BN,PRK,SALT,&LIKM);
}
...
static void LabeledExpand(octet *OKM,octet *PRK,octet *SUITE_ID,char *label,octet *INFO,int L)
{
    ...
    HKDF_Expand(MC_SHA2,HASH_TYPE_FP512BN,OKM,L,PRK,&LINFO);
}
...
static void ExtractAndExpand(int config_id,octet *OKM,octet *DH,octet *CONTEXT)
{
    ...
    LabeledExpand(OKM, &PRK,&SUITE_ID,(char *)"shared_secret",CONTEXT,HASH_TYPE_FP512BN);  
}

int DeriveKeyPair_FP512BN(int config_id,octet *SK,octet *PK,octet *SEED)
{
    int counter=0;
    char prk[MAX_HASH];
    octet PRK={0,sizeof(prk),prk};
    ...
    { // RFC7748
        ...
        OCT_clear(SK);
        while (!ECP_FP512BN_IN_RANGE(SK) && counter<256)
        {
            OCT_empty(&INFO);
    ...
    ECP_FP512BN_KEY_PAIR_GENERATE(NULL, SK, PK);
    if (kem==32 || kem==33)
        OCT_reverse(PK);
...
void HPKE_FP512BN_Encap(int config_id,octet *skE,octet *Z,octet *pkE,octet *pkR)
{
    int res,kem;
    char dh[POINT];
    ...
    {
        OCT_reverse(pkR);
        res=ECP_FP512BN_SVDP_DH(skE, pkR, &DH, 0);
        OCT_reverse(pkR);
        OCT_reverse(&DH);
    } else {
        res=ECP_FP512BN_SVDP_DH(skE, pkR, &DH, 0);
...
void HPKE_FP512BN_Decap(int config_id,octet *skR,octet *Z,octet *pkE,octet *pkR)
{
    int res,kem;
    char dh[POINT];
    octet DH={0,sizeof(dh),dh};
    ...
    {
        OCT_reverse(pkE);
        ECP_FP512BN_SVDP_DH(skR, pkE, &DH, 0);
        OCT_reverse(pkE);
        OCT_reverse(&DH);
    } else {
        ECP_FP512BN_SVDP_DH(skR, pkE, &DH, 0);
...
void HPKE_FP512BN_AuthEncap(int config_id,octet *skE,octet *skS,octet *Z,octet *pkE,octet *pkR,octet *pkS)
{
    int res,kem;
    char dh[2*POINT];
    octet DH={0,sizeof(dh),dh};
    ...
    {
        OCT_reverse(pkR);
        ECP_FP512BN_SVDP_DH(skE, pkR, &DH,0);
        ECP_FP512BN_SVDP_DH(skS, pkR, &DH1,0);
        OCT_reverse(pkR);
        OCT_reverse(&DH);
        OCT_reverse(&DH1);
    } else {
...
void HPKE_FP512BN_AuthDecap(int config_id,octet *skR,octet *Z,octet *pkE,octet *pkR,octet *pkS)
{
    int res,kem;
    char dh[2*POINT];
    octet DH={0,sizeof(dh),dh};
    ...
    if (kem==32 || kem==33) {
        ...
        OCT_reverse(pkS);
        ECP_FP512BN_SVDP_DH(skR, pkE, &DH,0);
        ECP_FP512BN_SVDP_DH(skR, pkS, &DH1,0);
        OCT_reverse(pkE);
        OCT_reverse(pkS);
        OCT_reverse(&DH);
        OCT_reverse(&DH1);
...
void HPKE_FP512BN_KeySchedule(int config_id,octet *key,octet *nonce,octet *exp_secret,int mode,octet *Z,octet *info,octet *psk,octet *pskID)
{
    ...
    LabeledExpand(key,&secret,&SUITE_ID,(char *)"key",&CONTEXT,AESKEY_FP512BN);
    LabeledExpand(nonce,&secret,&SUITE_ID,(char *)"base_nonce",&CONTEXT,12);
    if (exp_secret!=NULL)
        LabeledExpand(exp_secret,&secret,&SUITE_ID,(char *)"exp",&CONTEXT,HASH_TYPE_FP512BN);
}
