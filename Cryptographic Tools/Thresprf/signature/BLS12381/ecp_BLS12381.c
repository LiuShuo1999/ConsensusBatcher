...
int ECP_BLS12381_set(ECP_BLS12381 *P, BIG_384_29 x)
{
    //BIG_384_29 m, b;
    FP_BLS12381 rhs;
    //BIG_384_29_rcopy(m, Modulus_BLS12381);
...
int ECP_BLS12381_get(BIG_384_29 x, ECP_BLS12381 *P)
{
    ECP_BLS12381 W;
...
/* SU=16 */
int ECP_BLS12381_get(BIG_384_29 x, BIG_384_29 y, ECP_BLS12381 *P)
{
    ECP_BLS12381 W;
    ECP_BLS12381_copy(&W, P);
    ECP_BLS12381_affine(&W);
    ...
    return FP_BLS12381_sign(&(W.x));
}

/* Set P=(x,{y}) */
/* SU=96 */
int ECP_BLS12381_set(ECP_BLS12381 *P, BIG_384_29 x, BIG_384_29 y)
{
    FP_BLS12381 rhs, y2;
...
/* SU=136 */
int ECP_BLS12381_setx(ECP_BLS12381 *P, BIG_384_29 x, int s)
{
...
{   /* multiply point by curves cofactor */
    ...
    BIG_384_29_rcopy(c, CURVE_Cof_BLS12381);
...
/*
void ECP_BLS12381_hap2point(ECP_BLS12381 *P,BIG_384_29 h)
{
    BIG_384_29 x;
    BIG_384_29_copy(x,h);
...
// Elligator 2
    ...
    //BIG_384_29_zero(a); BIG_384_29_inc(a,CURVE_A_BLS12381); BIG_384_29_norm(a); FP_BLS12381_nres(&A,a);
...
// Elligator 2 - map to Montgomery, place point, map back
    int qres,ne,rfc,qnr;
    BIG_384_29 x,y;
    FP_BLS12381 X1,X2,t,w,one,A,w1,w2,B,Y,K,D,hint;
    FP_BLS12381_one(&one);
...
// Figure out RHS of Montgomery curve in rational form gx1/d^3
    ...
    FP_BLS12381_imul(&w1,&B,qnr);       // now for gx2 = Z.u^2.gx1
    FP_BLS12381_rcopy(&w,CURVE_HTPC_BLS12381);   // qnr^C3  
    FP_BLS12381_mul(&w,&w,&hint);    // modify hint for gx2
    FP_BLS12381_mul(&w2,&D,h);

    FP_BLS12381_cmove(&X1,&X2,1-qres);  // pick correct one
    FP_BLS12381_cmove(&B,&w1,1-qres);
    FP_BLS12381_cmove(&hint,&w,1-qres);
    FP_BLS12381_cmove(&D,&w2,1-qres);
...
// x^3+Ad^2x+Bd^3
        ...
        FP_BLS12381_mul(&D,&D2,&t);
        FP_BLS12381_imul(&t,&w,RIADZ_BLS12381);
        FP_BLS12381_rcopy(&X1,CURVE_HTPC_BLS12381);     
        FP_BLS12381_mul(&X1,&X1,&hint); // modify hint

        FP_BLS12381_cmove(&X2,&X3,1-qr); 
        FP_BLS12381_cmove(&D2,&D,1-qr);
        FP_BLS12381_cmove(&w,&t,1-qr);
        FP_BLS12381_cmove(&hint,&X1,1-qr);
...
void ECP_BLS12381_mapit(ECP_BLS12381 *P, octet *W)
{
    ...
    BIG_384_29_dfromBytesLen(dx, W->val,W->len);
    BIG_384_29_dmod(x, dx, q);
...
void ECP_BLS12381_toOctet(octet *W, ECP_BLS12381 *P, bool compress)
{
...
#if (MBITS-1)%8 <= 4
#ifdef ALLOW_ALT_COMPRESS_BLS12381
    alt=true;
#endif
#endif
    if (alt)
    {
        ...
        if (compress)
        {
            W->len = MODBYTES_384_29;
            W->val[0]|=0x80;
            if (FP_BLS12381_islarger(&(P->y))==1) W->val[0]|=0x20;
        } else {
            W->len = 2 * MODBYTES_384_29;
            BIG_384_29_toBytes(&(W->val[MODBYTES_384_29]), y);
        }
    } else {
        ...
        {
            W->val[0] = 0x02;
            if (FP_BLS12381_sign(&(P->y)) == 1) W->val[0] = 0x03;
            W->len = MODBYTES_384_29 + 1;
        } else {
            W->val[0] = 0x04;
...
int ECP_BLS12381_fromOctet(ECP_BLS12381 *P, octet *W)
{
#if CURVETYPE_BLS12381==MONTGOMERY
    BIG_384_29 x;
    BIG_384_29_fromBytes(x, &(W->val[0]));
    ...
    if (alt)
    {
        ...
        if ((typ&0x80)==0)
        {
            BIG_384_29_fromBytes(y, &(W->val[MODBYTES_384_29]));
            if (ECP_BLS12381_set(P, x, y)) return 1;
            return 0;
        } else {
            if (!ECP_BLS12381_setx(P,x,0)) return 0;
            sgn=(typ&0x20)>>5;
            cmp=FP_BLS12381_islarger(&(P->y));
            if ((sgn==1 && cmp!=1) || (sgn==0 && cmp==1)) ECP_BLS12381_neg(P);
            return 1;
        }
...
void ECP_BLS12381_mul(ECP_BLS12381 *P,BIG_384_29 e)
{
    ECP_BLS12381_clmul(P,e,e);
}

// .. but this one does not (typically set maxe=r)
// Set P=e*P 
void ECP_BLS12381_clmul(ECP_BLS12381 *P, BIG_384_29 e, BIG_384_29 maxe)
{
    int max;
    BIG_384_29 cm;
#if CURVETYPE_BLS12381==MONTGOMERY
    ...
    BIG_384_29_or(cm,e,maxe);
    max=BIG_384_29_nbits(cm);
...
void ECP_BLS12381_muln(ECP_BLS12381 *P,int n,ECP_BLS12381 X[],BIG_384_29 e[])
{
    int i,j,k,nb;
    BIG_384_29 t,mt;
    ECP_BLS12381 S,R,B[16];
    ECP_BLS12381_inf(P);

    BIG_384_29_copy(mt,e[0]);  BIG_384_29_norm(mt);
    for (i=1;i<n;i++)
    { // find biggest
        BIG_384_29_copy(t,e[i]); BIG_384_29_norm(t);
        k=BIG_384_29_comp(t,mt);
        BIG_384_29_cmove(mt,t,(k+1)/2);
    }
    nb=(BIG_384_29_nbits(mt)+3)/4;
    for (int i=nb-1;i>=0;i--)
    { // Pippenger's algorithm
        for (j=0;j<16;j++)
            ECP_BLS12381_inf(&B[j]);
        for (j=0;j<n;j++)
        {
            BIG_384_29_copy(mt,e[j]); BIG_384_29_norm(mt);
            BIG_384_29_shr(mt,4*i);
            k=BIG_384_29_lastbits(mt,4);
            ECP_BLS12381_add(&B[k],&X[j]);
        }
        ECP_BLS12381_inf(&R); ECP_BLS12381_inf(&S);
        for (j=15;j>=1;j--)
        {
            ECP_BLS12381_add(&R,&B[j]);
...
void ECP_BLS12381_mul2(ECP_BLS12381 *P, ECP_BLS12381 *Q, BIG_384_29 e, BIG_384_29 f)
{
    BIG_384_29 te, tf, mt;
...
{
    BIG_384_29 x, y;
    BIG_384_29_rcopy(x, CURVE_Gx_BLS12381);
#if CURVETYPE_BLS12381!=MONTGOMERY
    BIG_384_29_rcopy(y, CURVE_Gy_BLS12381);
