...
//任务优先级
#define THRES_PRF_TASK_PRIO		2
//任务堆栈大小	
#define THRES_PRF_STK_SIZE 		4096  
//任务句柄
TaskHandle_t THRES_PRF_Task_Handler;
//任务函数
void THRES_PRF(void *pvParameters);
...
void start_task(void *pvParameters)
{
    ...
    //创建LED0任务
    xTaskCreate((TaskFunction_t )THRES_PRF,     	
                (const char*    )"THRES_PRF",   	
                (uint16_t       )THRES_PRF_STK_SIZE, 
                (void*          )NULL,				
                (UBaseType_t    )THRES_PRF_TASK_PRIO,	
                (TaskHandle_t*  )&THRES_PRF_Task_Handler);   
