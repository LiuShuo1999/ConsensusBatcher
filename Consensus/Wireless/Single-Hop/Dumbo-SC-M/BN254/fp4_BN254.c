...
{
    return (FP2_BN254_iszilch(&(x->a)) & FP2_BN254_iszilch(&(x->b)));
}
...
{
    return (FP2_BN254_isunity(&(x->a)) & FP2_BN254_iszilch(&(x->b)));
}
...
{
    return (FP2_BN254_equals(&(x->a), &(y->a)) & FP2_BN254_equals(&(x->b), &(y->b)));
}

// Is x lexically larger than p-x?
// return -1 for no, 0 if x=0, 1 for yes
int FP4_BN254_islarger(FP4_BN254 *x)
{
    int cmp;
    if (FP4_BN254_iszilch(x)) return 0;
    cmp=FP2_BN254_islarger(&(x->b));
    if (cmp!=0) return cmp;
    return FP2_BN254_islarger(&(x->a));
}

void FP4_BN254_toBytes(char *b,FP4_BN254 *x)
{
    FP2_BN254_toBytes(b,&(x->b));
    FP2_BN254_toBytes(&b[2*MODBYTES_256_28],&(x->a));
}

void FP4_BN254_fromBytes(FP4_BN254 *x,char *b)
{
    FP2_BN254_fromBytes(&(x->b),b);
    FP2_BN254_fromBytes(&(x->a),&b[2*MODBYTES_256_28]);
}
...
/* set FP4 from FP */
void FP4_BN254_from_FP(FP4_BN254 *w, FP_BN254 *x)
{
    FP2_BN254 t;
    FP2_BN254_from_FP(&t, x);
    FP4_BN254_from_FP2(w, &t);
}
...
int FP4_BN254_sign(FP4_BN254 *w)
{
    int p1,p2;
    p1=FP2_BN254_sign(&(w->a));
    p2=FP2_BN254_sign(&(w->b));
#ifdef BIG_ENDIAN_SIGN_BN254
    p2 ^= (p1 ^ p2)&FP2_BN254_iszilch(&(w->b));
    return p2;
#else
    p1 ^= (p1 ^ p2)&FP2_BN254_iszilch(&(w->a));
    return p1;
#endif
}
...
/* Set w=1/x */
/* SU= 160 */
void FP4_BN254_inv(FP4_BN254 *w, FP4_BN254 *x, FP_BN254 *h)
{
    FP2_BN254 t1, t2;
...
/* SU= 240 */
/*
void FP4_BN254_pow(FP4_BN254 *r, FP4_BN254* a, BIG_256_28 b)
{
    FP4_BN254 w;
    BIG_256_28 z, zilch;
    int bt;
...
void FP4_BN254_xtr_pow(FP4_BN254 *r, FP4_BN254 *x, BIG_256_28 n)
{
    int i, par, nb;
    BIG_256_28 v;
    FP2_BN254 w;
...
void FP4_BN254_xtr_pow2(FP4_BN254 *r, FP4_BN254 *ck, FP4_BN254 *cl, FP4_BN254 *ckml, FP4_BN254 *ckm2l, BIG_256_28 a, BIG_256_28 b)
{
    int i, f2;
    BIG_256_28 d, e, w;
...
void FP4_BN254_rand(FP4_BN254 *x,csprng *rng)
{
    FP2_BN254_rand(&(x->a),rng);
    FP2_BN254_rand(&(x->b),rng);
}

#if PAIRING_FRIENDLY_BN254 >= BLS24_CURVE

/* test for x a QR */
int FP4_BN254_qr(FP4_BN254 *x, FP_BN254 *h)
{ /* test x^(p^4-1)/2 = 1 */
    FP4_BN254 c;
    FP4_BN254_conj(&c,x);
    FP4_BN254_mul(&c,&c,x);

    return FP2_BN254_qr(&(c.a),h);
}

/* sqrt(a+xb) = sqrt((a+sqrt(a*a-n*b*b))/2)+x.b/(2*sqrt((a+sqrt(a*a-n*b*b))/2)) */

void FP4_BN254_sqrt(FP4_BN254 *r, FP4_BN254* x, FP_BN254 *h)
{
    FP2_BN254 a, b, s, t;
    FP4_BN254 nr;
    FP_BN254 hint,twk;
    ...
    FP4_BN254_copy(r, x);
    if (FP4_BN254_iszilch(x)) return;

    FP2_BN254_copy(&a, &(x->a));
    FP2_BN254_copy(&s, &(x->b));

    FP2_BN254_sqr(&s, &s); // s*=s
    FP2_BN254_sqr(&a, &a); // a*=a
    FP2_BN254_mul_ip(&s);
    FP2_BN254_norm(&s);
    FP2_BN254_sub(&a, &a, &s); // a-=txx(s)
    FP2_BN254_norm(&a);
    FP2_BN254_sqrt(&s, &a, h);

    FP2_BN254_copy(&t, &(x->a));

    FP2_BN254_add(&a, &t, &s);
    FP2_BN254_norm(&a);
    FP2_BN254_div2(&a, &a);

    FP2_BN254_div2(&b,&(r->b));                   // w1=b/2
    qr=FP2_BN254_qr(&a,&hint);                    // only exp! Cost=+1

// tweak hint - multiply old hint by Norm(1/Beta)^e where Beta is irreducible polynomial
    FP2_BN254_copy(&s,&a);
    FP_BN254_rcopy(&twk,TWK_BN254);
    FP_BN254_mul(&twk,&twk,&hint);
    FP2_BN254_div_ip(&s); FP2_BN254_norm(&s); // switch to other candidate

    FP2_BN254_cmove(&a,&s,1-qr);
    FP_BN254_cmove(&hint,&twk,1-qr);

    FP2_BN254_sqrt(&(r->a),&a,&hint);             // a=sqrt(w2)  Cost=+1
    FP2_BN254_inv(&s,&a,&hint);                  // w3=1/w2
    FP2_BN254_mul(&s,&s,&(r->a));                // w3=1/sqrt(w2)
    FP2_BN254_mul(&(r->b),&s,&b);                // b=(b/2)*1/sqrt(w2)
    FP2_BN254_copy(&t,&(r->a));

    FP2_BN254_cmove(&(r->a),&(r->b),1-qr);
    FP2_BN254_cmove(&(r->b),&t,1-qr);

    sgn=FP4_BN254_sign(r);
    FP4_BN254_neg(&nr,r); FP4_BN254_norm(&nr);
    FP4_BN254_cmove(r,&nr,sgn);
}

void FP4_BN254_div_i(FP4_BN254 *f)
{
    FP2_BN254 u, v;
    FP2_BN254_copy(&u, &(f->a));
    FP2_BN254_copy(&v, &(f->b));
    FP2_BN254_div_ip(&u);
    FP2_BN254_copy(&(f->a), &v);
    FP2_BN254_copy(&(f->b), &u);
#if TOWER_BN254 == POSITOWER
    FP4_BN254_neg(f, f);  // ***
    FP4_BN254_norm(f);
#endif
...
/*
void FP4_BN254_div_2i(FP4_BN254 *f)
{
    FP2_BN254 u,v;
    FP2_BN254_copy(&u,&(f->a));
    FP2_BN254_copy(&v,&(f->b));
