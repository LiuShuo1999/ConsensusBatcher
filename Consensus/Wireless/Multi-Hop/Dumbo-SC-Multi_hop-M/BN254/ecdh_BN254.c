...
int ECP_BN254_IN_RANGE(octet* S)
{
    BIG_256_28 r,s;
...
int ECP_BN254_KEY_PAIR_GENERATE(csprng *RNG, octet* S, octet *W)
{
    BIG_256_28 r, s;
    ECP_BN254 G;
    int res = 0;
    ...
    ECP_BN254_clmul(&G, s, r);
    ECP_BN254_toOctet(W, &G, false); /* To use point compression on public keys, change to true */

    return res;
}

/* Validate public key */
int ECP_BN254_PUBLIC_KEY_VALIDATE(octet *W)
{
    BIG_256_28 q, r, k;
    ECP_BN254 WP;
    int valid, nb;
    ...
    valid = ECP_BN254_fromOctet(&WP, W);
    ...
    {   /* Check point is not in wrong group */
        ...
        if (!BIG_256_28_isunity(k)) ECP_BN254_mul(&WP, k);
        if (ECP_BN254_isinf(&WP)) res = ECDH_INVALID_PUBLIC_KEY;
    }
...
int ECP_BN254_SVDP_DH(octet *S, octet *WD, octet *Z, int type)
{
    BIG_256_28 r, s, wx;
    int valid;
    ECP_BN254 W;
    ...
    BIG_256_28_fromBytes(s, S->val);
    valid = ECP_BN254_fromOctet(&W, WD);

    if (!valid) res = ECDH_ERROR;
    if (res == 0)
    {
        BIG_256_28_rcopy(r, CURVE_Order_BN254);
        ECP_BN254_clmul(&W, s, r);
        if (ECP_BN254_isinf(&W)) res = ECDH_ERROR;
        else
        {
#if CURVETYPE_BN254!=MONTGOMERY
            if (type>0)
            {
                if (type==1) ECP_BN254_toOctet(Z,&W,true);
                else ECP_BN254_toOctet(Z,&W,false);
                return res;
            }
            else
                ECP_BN254_get(wx, wx, &W);
#else
            ECP_BN254_get(wx, &W);
#endif
            Z->len = EFS_BN254;
...
int ECP_BN254_SP_DSA(int hlen, csprng *RNG, octet *K, octet *S, octet *F, octet *C, octet *D)
{
    char h[128];
    ...
    int blen = H.len;
    if (H.len > EGS_BN254) blen = EGS_BN254;
    ...
    if (RNG != NULL)
    {
        do
        {
            ...
            ECP_BN254_copy(&V, &G);
            ECP_BN254_clmul(&V, u, r);

            ECP_BN254_get(vx, vx, &V);
...
int ECP_BN254_VP_DSA(int hlen, octet *W, octet *F, octet *C, octet *D)
{
    char h[128];
    octet H = {0, sizeof(h), h};
    ...
    int blen = H.len;
    if (blen > EGS_BN254) blen = EGS_BN254;
    ...
    if (res == 0)
    {
        ...
        {
            ECP_BN254_mul2(&WP, &G, h2, f);
            if (ECP_BN254_isinf(&WP)) res = ECDH_ERROR;
            else
            {
                ECP_BN254_get(d, d, &WP);
                BIG_256_28_mod(d, r);
...
void ECP_BN254_ECIES_ENCRYPT(int hlen, octet *P1, octet *P2, csprng *RNG, octet *W, octet *M, int tlen, octet *V, octet *C, octet *T)
{

    int i, len;
    char z[EFS_BN254], vz[3 * EFS_BN254 + 1], k[2 * AESKEY_BN254], k1[AESKEY_BN254], k2[AESKEY_BN254], l2[8], u[EFS_BN254];
    ...
    if (ECP_BN254_KEY_PAIR_GENERATE(RNG, &U, V) != 0) return;
    if (ECP_BN254_SVDP_DH(&U, W, &Z, 0) != 0) return;

    OCT_copy(&VZ, V);
    OCT_joctet(&VZ, &Z);

    KDF2(MC_SHA2, hlen, &K, 2 * AESKEY_BN254, &VZ, P1);

    K1.len = K2.len = AESKEY_BN254;
    for (i = 0; i < AESKEY_BN254; i++)
    {
        K1.val[i] = K.val[i];
        K2.val[i] = K.val[AESKEY_BN254 + i];
    }
...
int ECP_BN254_ECIES_DECRYPT(int hlen, octet *P1, octet *P2, octet *V, octet *C, octet *T, octet *U, octet *M)
{

    int i, len;
    char z[EFS_BN254], vz[3 * EFS_BN254 + 1], k[2 * AESKEY_BN254], k1[AESKEY_BN254], k2[AESKEY_BN254], l2[8], tag[32];
    octet Z = {0, sizeof(z), z};
    octet VZ = {0, sizeof(vz), vz};
    octet K = {0, sizeof(k), k};
    octet K1 = {0, sizeof(k1), k1};
    ...
    if (ECP_BN254_SVDP_DH(U, V, &Z, 0) != 0) return 0;
