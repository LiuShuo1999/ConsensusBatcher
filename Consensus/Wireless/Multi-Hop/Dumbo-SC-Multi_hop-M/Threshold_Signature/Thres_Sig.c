...
void PRBC_SK_sign(int prbc_idx){
	...
	for(int i=0;i<Obj.len;i++){
		PRBC_share[prbc_idx][IDX][i] = Obj.val[i];
	}
...
void CBC_test(){
	...
	for(int k=1;k<=Nodes_N;k++){
		CBC_SK_sign(k,CBC_v[IDX],CBC_v_SIZE);
	}
...
void CBC_SK_sign(u8 id, u8 str[],u8 size){
	...
	if(id == IDX){
		for(int i=0;i<Obj.len;i++)
			My_CBC_Echo_Share[IDX][i] = Obj.val[i];
		ACK_CBC_Echo[IDX] = 1;
		My_CBC_Echo_Number++;
	}
...
//	ECP_BN158_copy(&SIG,&r[0]);	
	...
	char digest[50];for(int i=0;i<50;i++)digest[i] = NULL;
	char CBC_Str[100];for(int i=0;i<100;i++)CBC_Str[i] = NULL;
	for(int i=0;i<CBC_v_SIZE;i++)CBC_Str[i] = CBC_v[IDX][i];
	hash256 sh256;
	HASH256_init(&sh256);
	for (int i=0;CBC_Str[i]!=0;i++) HASH256_process(&sh256,CBC_Str[i]);
    HASH256_hash(&sh256,digest);
...
void VAL_CBC_SK_sign(u8 id){
	...
	if(id == IDX){
		for(int i=0;i<Obj.len;i++)
			My_VAL_CBC_Echo_Share[IDX][i] = Obj.val[i];
		ACK_VAL_CBC_Echo[IDX] = 1;
...
int VAL_CBC_PK_verify_sig_share(u8 id){
	u8 IDX;if(start_Global_Consensus)IDX = Group_ID;else IDX = ID;
	if(ACK_VAL_CBC_Number[IDX] != Nodes_N - Nodes_f){while(1);}
//	TIM9_Exceed_Times = 0;
//	TIM9->CNT = 0;
...
//	return True
	...
	for(int k=1;k<=Nodes_N;k++){
		if(ACK_VAL_CBC[IDX][k]){
			for(int i=0;i<Share_Size;i++){
				CBC_Str[packet_idx] = VAL_CBC_v[IDX][k][i];
				packet_idx++;
			}
		}
...
//	ECP_BN158_copy(&SIG,&r[0]);	
	...
	for(int i=0;i<OBJ.len;i++)
		VAL_CBC_Finish_Share[IDX][i] = ch[i];
