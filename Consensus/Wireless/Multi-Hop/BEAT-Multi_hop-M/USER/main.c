...
//	packet_idx += Share_Size;
	...
	//share
	for(int i=0;i<Share_Size;i++){
		Send_Data_buff[packet_idx + i] = ABA_share[aba][round][IDX][i];
	}
	packet_idx += Share_Size;
	//verify share z
	for(int i=0;i<BIG_Size;i++){
		Send_Data_buff[packet_idx + i] = ABA_Z_share[aba][round][IDX][i];
	}
	packet_idx += BIG_Size;
	//verify share c
	for(int i=0;i<BIG_Size;i++){
		Send_Data_buff[packet_idx + i] = ABA_C_share[aba][round][IDX][i];
	}
	packet_idx += BIG_Size;
