...
u8 IDX;if(start_Global_Consensus)IDX = Group_ID;else IDX = ID;
	if(ACK_ABA_share[aba][round][IDX])return;
	//h ** sk
	char digest[50];for(int i=0;i<50;i++)digest[i] = NULL;
	char Coin_Str[32];for(int i=0;i<32;i++)Coin_Str[i] = NULL;
	if(!start_Global_Consensus)sprintf((char*)Coin_Str,"Coin:%d%d%d",aba,round,Block[1][0][0]);
	else sprintf((char*)Coin_Str,"Global_Coin:%d%d%d",aba,round,Global_Block[1][0][3]);
	hash256 sh256;
	HASH256_init(&sh256);
	...
	for(int i=0;i<Obj.len;i++)
		ABA_share[aba][round][IDX][i] = Obj.val[i];
	if(!ACK_ABA_share[aba][round][IDX])ABA_share_Number[aba][round]++;
	ACK_ABA_share[aba][round][IDX] = 1;
	...
	BIG_160_28_fromBytesLen(ABA_C[aba][round][IDX], digest,32);
	BIG_160_28_mod(ABA_C[aba][round][IDX],modl);
	
	//z = s + self.SK * c
	BIG_160_28 tmp;
	BIG_160_28_modmul(tmp,SKs[IDX],ABA_C[aba][round][IDX],modl);
	BIG_160_28_modadd(ABA_Z[aba][round][IDX],s,tmp,modl);
	
	BIG_160_28_toBytes((char*)ABA_C_share[aba][round][IDX],ABA_C[aba][round][IDX]);
	BIG_160_28_toBytes((char*)ABA_Z_share[aba][round][IDX],ABA_Z[aba][round][IDX]);
	
	//Coin_PK_verify_sig_share(IDX,aba,round,ABA_share[aba][round][IDX]);
