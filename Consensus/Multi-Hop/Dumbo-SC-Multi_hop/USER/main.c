...
for(int w_id=0;w_id<2;w_id++){
	...
	if(Nodes_N <= 8){
		...
		u8 padding = 0;
		if(sig_num >= 2) padding = 30;
		if(sig_num < 2) padding = 60;
		if(W_flag) padding = 30;
...
//	if(ReceBuff[0] == 0x00||ReceBuff[0] > Nodes_N || ReceBuff[1] > 20 || ReceBuff[2] > Nodes_N){
	...
	if(packet_type == TypeRBC_INIT && verify_flag){
		...
		packet_idx += 2;
		for(int k=1;k<=Nodes_N;k++){
			if((sig_ack & (1<<(k-1))) != 0 && ACK_PRBC_share[k][s_id] == 0){// && ACK_PRBC_share[k][IDX] == 1){
				for(int i=0;i<Share_Size;i++){
		...
		for(int o=0;o<round_number;o++){
			...
			//BVAL[0]
			if(((bval0 & (1<<(IDX-1))) == 0) && BVAL[aba_id][round][0] == 1){
				round_flag = 1;
				//break;
			}
			//BVAL[1]
			if(((bval1 & (1<<(IDX-1))) == 0) && BVAL[aba_id][round][1] == 1){
				round_flag = 1;
				//break;
			}
			
			//AUX[0]
			if(((aux0 & (1<<(IDX-1))) == 0) && AUX[aba_id][round][0] == 1){
				round_flag = 1;
				//break;
			}
			//AUX[1]
			if(((aux1 & (1<<(IDX-1))) == 0) && AUX[aba_id][round][1] == 1){
				round_flag = 1;
				//break;
			}
			...
			//share_nack
			packet_idx += 4;
			if((buff[packet_idx] & (1<<(IDX-1))) == 0 && ACK_ABA_share[aba_id][round][IDX] == 1)round_flag = 1;	
