...
static void dom2(bool ph,int cl,octet *DOM) {
    OCT_jstring(DOM,(char *)"SigFP256BN no FP256BN collisions");
    if (ph)
        OCT_jbyte(DOM,1,1);
    else
        OCT_jbyte(DOM,0,1);
...
static void dom4(bool ph,int cl,octet *DOM) {
    OCT_jstring(DOM,(char *)"SigFP256BN");
    if (ph)
        OCT_jbyte(DOM,1,1);
    else
        OCT_jbyte(DOM,0,1);
...
{
#if CURVE_SECURITY_FP256BN <= 128  // for ed25519?
    hash512 sh512;
    HASH512_init(&sh512);
...
static int encode_int(BIG_256_28 x,char *ei) {
    ...
    if (8*MODBYTES_256_28==MBITS_FP256BN) index=1; // extra byte needed for compression        
...
// encode point
static void encode(ECP_FP256BN *P,octet *W) {
    BIG_256_28 x,y;
    int b,index=0;
...
static void decode(octet *W,ECP_FP256BN *P) {
    BIG_256_28 X,Y;
    FP_FP256BN x,d,t,one,hint;
    int sign=0;  // LSB of x

    sign=decode_int(true,W->val,Y);
    FP_FP256BN_nres(&x,Y); FP_FP256BN_sqr(&x,&x);
    FP_FP256BN_copy(&d,&x); FP_FP256BN_one(&one);
    FP_FP256BN_sub(&x,&x,&one);
    FP_FP256BN_norm(&x);
    FP_FP256BN_rcopy(&t, CURVE_B_FP256BN);
    FP_FP256BN_mul(&d,&d,&t);
#if CURVE_A_FP256BN == 1
    FP_FP256BN_sub(&d,&d,&one);
#else
    FP_FP256BN_add(&d,&d,&one);
#endif    
    FP_FP256BN_norm(&d);
// inverse square root trick for sqrt(x/d)
    FP_FP256BN_sqr(&t,&x);
    FP_FP256BN_mul(&x,&x,&t);
    FP_FP256BN_mul(&x,&x,&d);
    if (!FP_FP256BN_qr(&x,&hint))
    {
        ECP_FP256BN_inf(P);
        return;
    }
    FP_FP256BN_sqrt(&d,&x,&hint);
    FP_FP256BN_inv(&x,&x,&hint);
    FP_FP256BN_mul(&x,&x,&d);
    FP_FP256BN_mul(&x,&x,&t);
 ...
 */
int EDDSA_FP256BN_KEY_PAIR_GENERATE(csprng *RNG,octet* D,octet *Q)
{
    BIG_256_28 r, x, y, s;
    ECP_FP256BN G;
    char digest[128];
...
int EDDSA_FP256BN_SIGNATURE(bool ph,octet *D, octet *context,octet *M,octet *SIG)
{
    DBIG_256_28 dr;
    ...
    EDDSA_FP256BN_KEY_PAIR_GENERATE(NULL,D,&Q);

    BIG_256_28_rcopy(q, CURVE_Order_FP256BN);
    ECP_FP256BN_generator(&R);
...
bool EDDSA_FP256BN_VERIFY(bool ph,octet *Q,octet *context,octet *M,octet *SIG) 
{
    DBIG_256_28 du;
    BIG_256_28 q,t,su;
    ECP_FP256BN R,QD,G;
    ...
    decode(SIG,&R);
    if (ECP_FP256BN_isinf(&R)) return false;
    decode_int(false,&SIG->val[b],t);
    if (BIG_256_28_comp(t,q)>=0) return false;
    decode(Q,&QD);
    if (ECP_FP256BN_isinf(&QD)) return false;
    ...
    for (int i=0;i<lg;i++)
    { // use cofactor 2^c
        ECP_FP256BN_dbl(&G); ECP_FP256BN_dbl(&QD); ECP_FP256BN_dbl(&R);
    }

    ECP_FP256BN_neg(&QD);
    ECP_FP256BN_mul2(&G,&QD,t,su);

    if (!ECP_FP256BN_equals(&G,&R)) return false;
