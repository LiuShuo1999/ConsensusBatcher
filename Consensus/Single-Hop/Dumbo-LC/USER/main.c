...
//			//if(ABA_decided[k] == 1){
//				for(int i=0;i<Share_Size;i++){
//					Send_Data_buff[packet_idx + i] = Tx_shares_str[CBC_v[cur_aba_id][j]][ID-1][i]; 
//				}
//				packet_idx += Share_Size;
//			//}
//			
...
//						ACK_BVAL_Number[ID][1][1] = 1;
						Local_ABA_RBC_Init[ID][1][1][ID] = 1;
						Local_ABA_est[ID][1][1] = 1;
						Local_ABA_RBC_Echo_Number[ID][1][1][ID] = 1;
						Local_ABA_RBC_Echo[ID][1][1] |= (1<<(ID-1));
						Local_ABA_vset[ID][1][1][0] = 1;
						Local_ABA_vset[ID][1][1][1] = 1;
					}
...
//							}
						if(Local_ABA_RBC_Init[q][1][1][ID] == 0xFF){
//							est[m][1] = 1;
//							BVAL[m][1][1] = 1;
//							ACK_BVAL_Number[m][1][1] = 1;
							Local_ABA_RBC_Init[q][1][1][ID] = 0;
							Local_ABA_est[q][1][1] = 0;
							Local_ABA_RBC_Echo_Number[q][1][1][ID] = 1;
							Local_ABA_RBC_Echo[q][1][1] |= (1<<(ID-1));
								...
								if(j <= 2 && (Local_ABA_votes[aba_id][round][j][1] + Local_ABA_votes[aba_id][round][j][0] == Nodes_N - Nodes_f)){
									...
									if(j == 1){//phase 1
										if(votes0 == Nodes_N - Nodes_f || votes1 == Nodes_N - Nodes_f ){
											...
											// to next phase RBC
											Local_ABA_RBC_Init[aba_id][round][2][ID] = vote;
											Local_ABA_RBC_Echo_Number[aba_id][round][2][ID] = 1;
											Local_ABA_RBC_Echo[aba_id][round][2] |= (1<<(ID-1));
										...
										if(votes0 >= (Nodes_N/2) || votes1 >= (Nodes_N/2) ){
											...
											// to next phase RBC
											Local_ABA_RBC_Init[aba_id][round][3][ID] = vote;
											Local_ABA_RBC_Echo_Number[aba_id][round][3][ID] = 1;
											Local_ABA_RBC_Echo[aba_id][round][3] |= (1<<(ID-1));
										}else{
											//vset = {0,1}
											...
											// to next phase RBC
											Local_ABA_RBC_Init[aba_id][round][3][ID] = 3;
											Local_ABA_RBC_Echo_Number[aba_id][round][3][ID] = 1;
											Local_ABA_RBC_Echo[aba_id][round][3] |= (1<<(ID-1));
										}
									}
								...
								if(j == 3){
									if(Local_ABA_votes[aba_id][round][j][0] + Local_ABA_votes[aba_id][round][j][1] + Local_ABA_votes_3_NULL[i][j] == Nodes_N - Nodes_f){
										...
										if(votes0 == 2 * Nodes_f + 1 || votes1 == 2 * Nodes_f + 1){
											...
											// to next round RBC
											Local_ABA_RBC_Init[aba_id][round + 1][1][ID] = vote;
											Local_ABA_RBC_Echo_Number[aba_id][round + 1][1][ID] = 1;
											Local_ABA_RBC_Echo[aba_id][round + 1][1] |= (1<<(ID-1));
											...
											// to next round RBC
											Local_ABA_RBC_Init[aba_id][round + 1][1][ID] = Local_ABA_est[aba_id][round + 1][1];
											Local_ABA_RBC_Echo_Number[aba_id][round + 1][1][ID] = 1;
											Local_ABA_RBC_Echo[aba_id][round + 1][1] |= (1<<(ID-1));
										}
...
//				ACK_BVAL_Number[s_id][1][1] = 1;
				Local_ABA_RBC_Init[s_id][1][1][ID] = 1;
				Local_ABA_est[s_id][1][1] = 1;
				Local_ABA_RBC_Echo_Number[s_id][1][1][ID] = 1;
				Local_ABA_RBC_Echo[s_id][1][1] |= (1<<(ID-1));
					...
					if(Tx_shares_Number[k] >= Enc_Thres_k){
						...
						if(dec_tx_flag[k] == 0){
							...
							if(ID == 1 && dec_tx_number == dec_done_number && MY_Consensus_TIME==0){
								...
								sprintf((char*)Send_Data_buff,"Dumbo-LC-%d-%d-%d",Block_Num,MY_Consensus_TIME,dec_tx_number * Proposal_Size);
								LoRa_SendData(Send_Data_buff);
								while(1);
							}
						}
