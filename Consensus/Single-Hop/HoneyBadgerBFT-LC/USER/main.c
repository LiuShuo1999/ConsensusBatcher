...
//	
//	Tx_SK_decrypt_share(1,ID);
//	Tx_SK_decrypt_share(2,ID);
//	Tx_SK_decrypt_share(3,ID);
//	Tx_SK_decrypt_share(4,ID);
...
//							ACK_BVAL_Number[k][1][1] = 1;
							Local_ABA_RBC_Init[k][1][1][ID] = 1;
							Local_ABA_est[k][1][1] = 1;
							Local_ABA_RBC_Echo_Number[k][1][1][ID] = 1;
							Local_ABA_RBC_Echo[k][1][1] |= (1<<(ID-1));
...
//									ACK_BVAL_Number[m][1][1] = 1;
									Local_ABA_RBC_Init[m][1][1][ID] = 0;
									Local_ABA_est[m][1][1] = 0;
									Local_ABA_RBC_Echo_Number[m][1][1][ID] = 1;
									Local_ABA_RBC_Echo[m][1][1] |= (1<<(ID-1));
					...
					if(Tx_shares_Number[k] >= Enc_Thres_k){
						...
						if(dec_tx_flag[k] == 0){
							...
							if(ID == 1 && dec_tx_number == dec_done_number && MY_Consensus_TIME==0){
								...
								sprintf((char*)Send_Data_buff,"HBBFT-LC-%d-%d-%d",Block_Num,MY_Consensus_TIME,dec_tx_number * Proposal_Size);
								LoRa_SendData(Send_Data_buff);
								while(1);
							}
						}
