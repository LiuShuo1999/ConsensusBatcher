...
//	if(ReceBuff[0] == 0x00||ReceBuff[0] > Nodes_N || ReceBuff[1] > 20 || ReceBuff[2] > Nodes_N){
	...
	if(packet_type == TypeSTATE && verify_flag && RBC_init_Number >= Nodes_N - 1){
		...
		for(int o=0;o<round_number;o++){
			...
			for(int k=1;k<=MAX_ABA;k++){//ABA
				
				if(ABA_R[k]>round) continue;
				
				if((buff[tmp_packet_idx + (s_id - 1) * 2] & (1<<(k-1))) && !ACK_BVAL[k][round][s_id][0]){ACK_BVAL[k][round][s_id][0] = 1;ACK_BVAL_Number[k][round][0]++;}
				if((buff[tmp_packet_idx + (s_id - 1) * 2 + 1] & (1<<(k-1))) && !ACK_BVAL[k][round][s_id][1]){ACK_BVAL[k][round][s_id][1] = 1;ACK_BVAL_Number[k][round][1]++;}
				if((buff[tmp_packet_idx + (s_id - 1) * 2 + 2 * Nodes_N] & (1<<(k-1))) && !ACK_AUX[k][round][s_id][0]){ACK_AUX[k][round][s_id][0] = 1;ACK_AUX_Number[k][round][0]++;}
				if((buff[tmp_packet_idx + (s_id - 1) * 2 + 1 + 2 * Nodes_N] & (1<<(k-1))) && !ACK_AUX[k][round][s_id][1]){ACK_AUX[k][round][s_id][1] = 1;ACK_AUX_Number[k][round][1]++;}
					...
					if(Tx_shares_Number[k] >= Enc_Thres_k){
						...
						if(dec_tx_flag[k] == 0){
							...
							if(ID == 1 && dec_tx_number == dec_done_number && MY_Consensus_TIME==0){
								...
								sprintf((char*)Send_Data_buff,"HBBFT-SC-%d-%d-%d",Block_Num,MY_Consensus_TIME,dec_tx_number * Proposal_Size);
								LoRa_SendData(Send_Data_buff);
								while(1);
							}
