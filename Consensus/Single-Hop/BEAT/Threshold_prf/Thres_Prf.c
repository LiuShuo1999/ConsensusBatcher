...
void Coin_SK_sign(int round){
	...
	BIG_160_28_fromBytesLen(ABA_C[round][ID], digest,32);
	BIG_160_28_mod(ABA_C[round][ID],modl);
	
	//z = s + self.SK * c
	BIG_160_28 tmp;
	BIG_160_28_modmul(tmp,SKs[ID],ABA_C[round][ID],modl);
	BIG_160_28_modadd(ABA_Z[round][ID],s,tmp,modl);
	
	BIG_160_28_toBytes((char*)ABA_C_share[round][ID],ABA_C[round][ID]);
	BIG_160_28_toBytes((char*)ABA_Z_share[round][ID],ABA_Z[round][ID]);
