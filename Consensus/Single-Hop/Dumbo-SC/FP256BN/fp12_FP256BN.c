...
static void FP12_FP256BN_select(FP12_FP256BN *f, FP12_FP256BN g[], sign32 b)
{
    FP12_FP256BN invf;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
    ...
    FP12_FP256BN_cmove(f, &g[0], teq(babs, 0)); // conditional move
    FP12_FP256BN_cmove(f, &g[1], teq(babs, 1));
    FP12_FP256BN_cmove(f, &g[2], teq(babs, 2));
    FP12_FP256BN_cmove(f, &g[3], teq(babs, 3));
    FP12_FP256BN_cmove(f, &g[4], teq(babs, 4));
    FP12_FP256BN_cmove(f, &g[5], teq(babs, 5));
    FP12_FP256BN_cmove(f, &g[6], teq(babs, 6));
    FP12_FP256BN_cmove(f, &g[7], teq(babs, 7));

    FP12_FP256BN_copy(&invf, f);
    FP12_FP256BN_conj(&invf, &invf); // 1/f
    FP12_FP256BN_cmove(f, &invf, (int)(m & 1));
}
...
void FP12_FP256BN_zero(FP12_FP256BN *w)
{
    FP4_FP256BN_zero(&(w->a));
    FP4_FP256BN_zero(&(w->b));
    FP4_FP256BN_zero(&(w->c));
    w->type = FP_ZILCH;
}
...
{
    if (FP4_FP256BN_equals(&(x->a), &(y->a)) && FP4_FP256BN_equals(&(x->b), &(y->b)) && FP4_FP256BN_equals(&(x->c), &(y->c)))
        return 1;
    return 0;
}
...
void FP12_FP256BN_mul(FP12_FP256BN *w, FP12_FP256BN *y)
{
    ...
    FP4_FP256BN_add(&(w->b), &z1, &t1); // z1=z1-z2
...
void FP12_FP256BN_ssmul(FP12_FP256BN *w, FP12_FP256BN *y)
{
    FP4_FP256BN z0, z1, z2, z3, t0, t1;
    if (w->type == FP_UNITY)
    {
        FP12_FP256BN_copy(w, y);
        return;
    }
    if (y->type == FP_UNITY)
        return;
    ...
    if (y->type >= FP_SPARSE)
    {
        FP4_FP256BN_mul(&z0, &(w->a), &(y->a)); // xa.ya   always 11x11

#if SEXTIC_TWIST_FP256BN == M_TYPE
        if (y->type == FP_SPARSE || w->type == FP_SPARSE)
        {
            FP2_FP256BN_mul(&z2.b, &(w->b).b, &(y->b).b);
            FP2_FP256BN_zero(&z2.a);
            if (y->type != FP_SPARSE)
                FP2_FP256BN_mul(&z2.a, &(w->b).b, &(y->b).a);
            if (w->type != FP_SPARSE)
                FP2_FP256BN_mul(&z2.a, &(w->b).a, &(y->b).b);
            FP4_FP256BN_times_i(&z2);
        }
        else
#endif
            FP4_FP256BN_mul(&z2, &(w->b), &(y->b)); // xb.yb  could be 00x00 or 01x01 or or 10x10 or 11x00 or 11x10 or 11x01 or 11x11

        FP4_FP256BN_add(&t0, &(w->a), &(w->b)); // (xa+xb)
        FP4_FP256BN_add(&t1, &(y->a), &(y->b)); // (ya+yb)

        FP4_FP256BN_norm(&t0);
        FP4_FP256BN_norm(&t1);

        FP4_FP256BN_mul(&z1, &t0, &t1); // (xa+xb)(ya+yb)  always 11x11
        FP4_FP256BN_add(&t0, &(w->b), &(w->c)); // (xb+xc)
        FP4_FP256BN_add(&t1, &(y->b), &(y->c)); // (yb+yc)

        FP4_FP256BN_norm(&t0);
        FP4_FP256BN_norm(&t1);

        FP4_FP256BN_mul(&z3, &t0, &t1); // (xb+xc)(yb+yc)   could be anything...
        FP4_FP256BN_neg(&t0, &z0);      // -(xa.ya)
        FP4_FP256BN_neg(&t1, &z2);      // -(xb.yb)

        FP4_FP256BN_add(&z1, &z1, &t0);
        FP4_FP256BN_add(&(w->b), &z1, &t1); // /wb = (xa+xb)(ya+yb) -(xa.ya) -(xb.yb)                     = xa.yb + xb.ya

        FP4_FP256BN_add(&z3, &z3, &t1);      // (xb+xc)(yb+yc) -(xb.yb)
        FP4_FP256BN_add(&z2, &z2, &t0);      // (xb.yb) - (xa.ya)

        FP4_FP256BN_add(&t0, &(w->a), &(w->c)); // (xa+xc)
        FP4_FP256BN_add(&t1, &(y->a), &(y->c)); // (ya+yc)

        FP4_FP256BN_norm(&t0);
        FP4_FP256BN_norm(&t1);

        FP4_FP256BN_mul(&t0, &t1, &t0); // (xa+xc)(ya+yc)    always 11x11
        FP4_FP256BN_add(&z2, &z2, &t0); // (xb.yb) - (xa.ya) + (xa+xc)(ya+yc)

#if SEXTIC_TWIST_FP256BN == D_TYPE
        if (y->type == FP_SPARSE || w->type == FP_SPARSE)
        {
            FP2_FP256BN_mul(&t0.a, &(w->c).a, &(y->c).a);
            FP2_FP256BN_zero(&t0.b);
            if (y->type != FP_SPARSE)
                FP2_FP256BN_mul(&t0.b, &(w->c).a, &(y->c).b);
            if (w->type != FP_SPARSE)
                FP2_FP256BN_mul(&t0.b, &(w->c).b, &(y->c).a);
        }
        else
#endif
            FP4_FP256BN_mul(&t0, &(w->c), &(y->c)); // (xc.yc)  could be anything

        FP4_FP256BN_neg(&t1, &t0);            // -(xc.yc)

        FP4_FP256BN_add(&(w->c), &z2, &t1);     // wc = (xb.yb) - (xa.ya) + (xa+xc)(ya+yc) - (xc.yc)    =  xb.yb + xc.ya + xa.yc
        FP4_FP256BN_add(&z3, &z3, &t1);         // (xb+xc)(yb+yc) -(xb.yb) - (xc.yc)                    =  xb.yc + xc.yb
        FP4_FP256BN_times_i(&t0);               // i.(xc.yc)
        FP4_FP256BN_add(&(w->b), &(w->b), &t0); // wb = (xa+xb)(ya+yb) -(xa.ya) -(xb.yb) +i(xc.yc)
        FP4_FP256BN_norm(&z3);
        FP4_FP256BN_times_i(&z3);               // i[(xb+xc)(yb+yc) -(xb.yb) - (xc.yc)]                 = i(xb.yc + xc.yb)
        FP4_FP256BN_add(&(w->a), &z0, &z3);     // wa = xa.ya + i(xb.yc + xc.yb)
    } else {
        if (w->type == FP_SPARSER || w->type == FP_SPARSEST)
        {
            FP12_FP256BN_smul(w, y);
            return;
        }
// dense by sparser - 13m
        ...
        if (y->type == FP_SPARSEST)
            FP4_FP256BN_qmul(&z2, &(w->b), &(y->b).a.a);
        else
        ...
        FP4_FP256BN_norm(&z3);
        if (y->type == FP_SPARSEST)
            FP4_FP256BN_qmul(&z3, &z3, &(y->b).a.a);
        else
        ...
        if (y->type == FP_SPARSEST)
            FP4_FP256BN_qmul(&z3, &t0, &(y->c).b.a);
        else
        ...
        if (y->type == FP_SPARSEST)
            FP4_FP256BN_qmul(&t0, &(w->c), &(y->c).b.a);
        else
...
void FP12_FP256BN_smul(FP12_FP256BN *w, FP12_FP256BN *y)
{
    FP2_FP256BN w1, w2, w3, ta, tb, tc, td, te, t;

//  if (type==D_TYPE)
//  {
#if SEXTIC_TWIST_FP256BN == D_TYPE
    FP2_FP256BN_mul(&w1, &(w->a).a, &(y->a).a); // A1.A2
    FP2_FP256BN_mul(&w2, &(w->a).b, &(y->a).b); // B1.B2

    if (y->type == FP_SPARSEST || w->type == FP_SPARSEST)
    {
        if (y->type == FP_SPARSEST && w->type == FP_SPARSEST)
        {
            FP_FP256BN_mul(&w3.a, &(w->b).a.a, &(y->b).a.a);
            FP_FP256BN_zero(&w3.b);
        } else {
            if (y->type != FP_SPARSEST)
                FP2_FP256BN_pmul(&w3, &(y->b).a, &(w->b).a.a);
            if (w->type != FP_SPARSEST)
                FP2_FP256BN_pmul(&w3, &(w->b).a, &(y->b).a.a);
        }
    }
    else
        FP2_FP256BN_mul(&w3, &(w->b).a, &(y->b).a); // C1.C2

    FP2_FP256BN_add(&ta, &(w->a).a, &(w->a).b); // A1+B1
    FP2_FP256BN_add(&tb, &(y->a).a, &(y->a).b); // A2+B2
    FP2_FP256BN_norm(&ta);
    FP2_FP256BN_norm(&tb);
    FP2_FP256BN_mul(&tc, &ta, &tb);         // (A1+B1)(A2+B2)
    FP2_FP256BN_add(&t, &w1, &w2);
    FP2_FP256BN_neg(&t, &t);
    FP2_FP256BN_add(&tc, &tc, &t);          // (A1+B1)(A2+B2)-A1.A2-B1*B2 =  (A1.B2+A2.B1)

    FP2_FP256BN_add(&ta, &(w->a).a, &(w->b).a); // A1+C1
    FP2_FP256BN_add(&tb, &(y->a).a, &(y->b).a); // A2+C2
    FP2_FP256BN_norm(&ta);
    FP2_FP256BN_norm(&tb);
    FP2_FP256BN_mul(&td, &ta, &tb);         // (A1+C1)(A2+C2)
    FP2_FP256BN_add(&t, &w1, &w3);
    FP2_FP256BN_neg(&t, &t);
    FP2_FP256BN_add(&td, &td, &t);          // (A1+C1)(A2+C2)-A1.A2-C1*C2 =  (A1.C2+A2.C1)

    FP2_FP256BN_add(&ta, &(w->a).b, &(w->b).a); // B1+C1
    FP2_FP256BN_add(&tb, &(y->a).b, &(y->b).a); // B2+C2
    FP2_FP256BN_norm(&ta);
    FP2_FP256BN_norm(&tb);
    FP2_FP256BN_mul(&te, &ta, &tb);         // (B1+C1)(B2+C2)
    FP2_FP256BN_add(&t, &w2, &w3);
    FP2_FP256BN_neg(&t, &t);
    FP2_FP256BN_add(&te, &te, &t);          // (B1+C1)(B2+C2)-B1.B2-C1*C2 =  (B1.C2+B2.C1)

    FP2_FP256BN_mul_ip(&w2);
    FP2_FP256BN_add(&w1, &w1, &w2);
    FP4_FP256BN_from_FP2s(&(w->a), &w1, &tc);
    FP4_FP256BN_from_FP2s(&(w->b), &td, &te); // only norm these 2
    FP4_FP256BN_from_FP2(&(w->c), &w3);
...
//  } else {
    ...
    if (y->type == FP_SPARSEST || w->type == FP_SPARSEST)
    {
        if (y->type == FP_SPARSEST && w->type == FP_SPARSEST)
        {
            FP_FP256BN_mul(&w3.a, &(w->c).b.a, &(y->c).b.a);
            FP_FP256BN_zero(&w3.b);
        } else {
            if (y->type != FP_SPARSEST)
                FP2_FP256BN_pmul(&w3, &(y->c).b, &(w->c).b.a);
            if (w->type != FP_SPARSEST)
                FP2_FP256BN_pmul(&w3, &(w->c).b, &(y->c).b.a);
        }
    }
    else
        FP2_FP256BN_mul(&w3, &(w->c).b, &(y->c).b); // F1.F2
    ...
    FP2_FP256BN_add(&ta, &(w->a).a, &(w->c).b); // A1+F1
    FP2_FP256BN_add(&tb, &(y->a).a, &(y->c).b); // A2+F2
    FP2_FP256BN_norm(&ta);
    FP2_FP256BN_norm(&tb);
    FP2_FP256BN_mul(&td, &ta, &tb);         // (A1+F1)(A2+F2)
    FP2_FP256BN_add(&t, &w1, &w3);
    FP2_FP256BN_neg(&t, &t);
    FP2_FP256BN_add(&td, &td, &t);          // (A1+F1)(A2+F2)-A1.A2-F1*F2 =  (A1.F2+A2.F1)

    FP2_FP256BN_add(&ta, &(w->a).b, &(w->c).b); // B1+F1
    FP2_FP256BN_add(&tb, &(y->a).b, &(y->c).b); // B2+F2
    FP2_FP256BN_norm(&ta);
    FP2_FP256BN_norm(&tb);
    FP2_FP256BN_mul(&te, &ta, &tb);         // (B1+F1)(B2+F2)
    FP2_FP256BN_add(&t, &w2, &w3);
    FP2_FP256BN_neg(&t, &t);
    FP2_FP256BN_add(&te, &te, &t);          // (B1+F1)(B2+F2)-B1.B2-F1*F2 =  (B1.F2+B2.F1)

    FP2_FP256BN_mul_ip(&w2);
    FP2_FP256BN_add(&w1, &w1, &w2);
    FP4_FP256BN_from_FP2s(&(w->a), &w1, &tc);

    FP2_FP256BN_mul_ip(&w3);
    FP2_FP256BN_norm(&w3);
    FP4_FP256BN_from_FP2H(&(w->b), &w3);

    FP2_FP256BN_norm(&te);
    FP2_FP256BN_mul_ip(&te);
    FP4_FP256BN_from_FP2s(&(w->c), &te, &td);
...
void FP12_FP256BN_compow(FP4_FP256BN *c, FP12_FP256BN *x, BIG_256_28 e, BIG_256_28 r)
{
    FP12_FP256BN g1, g2;
    ...
    BIG_256_28_rcopy(a, Fra_FP256BN);
    BIG_256_28_rcopy(b, Frb_FP256BN);
...
void FP12_FP256BN_pow(FP12_FP256BN *r, FP12_FP256BN *a, BIG_256_28 b)
{
    FP12_FP256BN w, sf;
    BIG_256_28 b1, b3;
    int i, nb, bt;
    ...
    FP12_FP256BN_copy(&sf, a);
    FP12_FP256BN_norm(&sf);
    FP12_FP256BN_copy(&w, &sf);
    ...
    for (i = nb - 2; i >= 1; i--)
    {
        ...
        if (bt == 1)
            FP12_FP256BN_mul(&w, &sf);
        if (bt == -1)
        {
            FP12_FP256BN_conj(&sf, &sf);
            FP12_FP256BN_mul(&w, &sf);
            FP12_FP256BN_conj(&sf, &sf);
        }
    }
...
void FP12_FP256BN_pow4(FP12_FP256BN *p, FP12_FP256BN *q, BIG_256_28 u[4])
{
    int i, j, k, nb, pb, bt;
    FP12_FP256BN g[8], r;
    BIG_256_28 t[4], mt;
...
// Precomputed table
    FP12_FP256BN_copy(&g[0], &q[0]); // q[0]
    FP12_FP256BN_copy(&g[1], &g[0]);
    FP12_FP256BN_mul(&g[1], &q[1]); // q[0].q[1]
    FP12_FP256BN_copy(&g[2], &g[0]);
    FP12_FP256BN_mul(&g[2], &q[2]); // q[0].q[2]
    FP12_FP256BN_copy(&g[3], &g[1]);
    FP12_FP256BN_mul(&g[3], &q[2]); // q[0].q[1].q[2]
    FP12_FP256BN_copy(&g[4], &g[0]);
    FP12_FP256BN_mul(&g[4], &q[3]); // q[0].q[3]
    FP12_FP256BN_copy(&g[5], &g[1]);
    FP12_FP256BN_mul(&g[5], &q[3]); // q[0].q[1].q[3]
    FP12_FP256BN_copy(&g[6], &g[2]);
    FP12_FP256BN_mul(&g[6], &q[3]); // q[0].q[2].q[3]
    FP12_FP256BN_copy(&g[7], &g[3]);
    FP12_FP256BN_mul(&g[7], &q[3]); // q[0].q[1].q[2].q[3]
...
// Main loop
    FP12_FP256BN_select(p, g, 2 * w[nb - 1] + 1);
    for (i = nb - 2; i >= 0; i--)
    {
        FP12_FP256BN_select(&r, g, 2 * w[i] + s[i]);
        FP12_FP256BN_usqr(p, p);
        FP12_FP256BN_mul(p, &r);
    }
// apply correction
    FP12_FP256BN_conj(&r, &q[0]);
    FP12_FP256BN_mul(&r, p);
    FP12_FP256BN_cmove(p, &r, pb);
...
void FP12_FP256BN_toOctet(octet *W, FP12_FP256BN *g)
{
    ...
    FP4_FP256BN_toBytes(&(W->val[0]),&(g->c));
    FP4_FP256BN_toBytes(&(W->val[4 * MODBYTES_256_28]),&(g->b));
    FP4_FP256BN_toBytes(&(W->val[8 * MODBYTES_256_28]),&(g->a));
}
...
{
    FP4_FP256BN_fromBytes(&(g->c),&(W->val[0]));
    FP4_FP256BN_fromBytes(&(g->b),&(W->val[4 * MODBYTES_256_28]));
    FP4_FP256BN_fromBytes(&(g->a),&(W->val[8 * MODBYTES_256_28]));
}

/* Move b to a if d=1 */
void FP12_FP256BN_cmove(FP12_FP256BN *f, FP12_FP256BN *g, int d)
{
    FP4_FP256BN_cmove(&(f->a), &(g->a), d);
    FP4_FP256BN_cmove(&(f->b), &(g->b), d);
    FP4_FP256BN_cmove(&(f->c), &(g->c), d);
    d = ~(d - 1);
